...
def ensure_embedded_outpost_host():
    ...
    try:
        ...
        # Also pin the browser host so external redirects resolve to the same
        # public URL (belt-and-braces; the proxy outpost uses authentik_host).
        if not cfg.get("authentik_host_browser"):
            cfg["authentik_host_browser"] = host
        outpost._config = cfg
        outpost.save(update_fields=["_config"])
        print(f"ensure_embedded_outpost_host: set authentik_host {current!r} -> {host!r}")
    except Exception as e:
...
if __name__ == "__main__":
    ...
    # Ensure akadmin is in Super Admins group and has correct email
    try:
        ...
        if user.email != admin_email:
            ...
            print(f"Set akadmin email to {admin_email}")
        ...
        if not group.users.filter(pk=user.pk).exists():
           ...
           print("Added akadmin to razzfazz.ai Super Admins")
        else:
           print("akadmin is already in razzfazz.ai Super Admins")
    except Exception as e:
        print(f"Error adding akadmin to Super Admins: {e}")
