...
# #54 — central password broker. We do NOT enable GLOBAL CSRF verification
# (enable_csrf with a before_request) because the pre-existing JSON prefs API
# (/api/prefs/*, /api/categories/*) is called via fetch WITHOUT a CSRF token —
# turning on global POST verification would break it. Instead we (a) expose the
# csrf_token() template helper app-wide (harmless), and (b) verify the token
# EXPLICITLY inside the broker's POST handler (verify_methods=() registers the
# context processor only, no before_request hook). The broker form is the one
# state-changing surface that needs CSRF.
...
enable_csrf(app, verify_methods=())  # context-processor only; no global hook
