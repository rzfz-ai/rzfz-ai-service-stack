...
def settings():
    ...
    if request.method == 'POST':
        ...
        # #182: model files (gpustack-data|speaches-data) follow the include_models
        # toggle, but the physical DB datadirs (postgres-data|valkey-data|
        # postgres-komodo-data) are ALWAYS excluded — they're captured logically
        # by core/backup/pre-backup.sh (pg_dumpall + valkey SAVE) and imported on
        # restore. Keeping the live datadirs out of the tar is what lets postgres
        # run with stop-during-backup=false (no nightly DB blip); re-including
        # them REQUIRES restoring the stop labels (see core/compose.yml, #157).
        exclude_parts = ([] if include_models else ['gpustack-data', 'speaches-data']) \
            + ['postgres-data', 'valkey-data', 'postgres-komodo-data']
        write_env_value('BACKUP_EXCLUDE_REGEXP', '|'.join(exclude_parts))
