...
from razzfazz_common.calver import sort_versions as calver_sort
...
def updates_check():
    ...
    #1312 review, finding 4: this route sat in EXEMPT_ROUTE_PATTERNS as a
    "remote manifest refresh, no local mutation", licensed by a comment that
    claimed image_checker audit-logs internally. It does not (`grep -c audit
    core/config/app/services/image_checker.py` is 0), and the route is not
    read-only: `fetch_remote_manifest` runs `git fetch` in STACK_ROOT and
    `_verify_and_persist` OVERWRITES config/manifests/versions.json. That is
    the file every later update decision is read from, so the refresh belongs
    in the trail like any other change to it.
    """
    ...
    current_app.audit_logger.log(
        'updates.check', user=user, source_ip=source_ip,
        category='updates', action='refresh_manifest',
        target='config/manifests/versions.json',
        detail=(f'Refreshed the image manifest from the remote: {message}' if ok
                else f'Manifest refresh FAILED: {message}'),
        risk='low', outcome='success' if ok else 'failure',
    )
    if not ok:
...
def gpustack_models():
    """LLM Models panel (HTMX fragment). #976: the source is the ACTIVE LLM
    backend — the LLM Manager when its profile is enabled (it fronts every
    worker; on a manager-only box GPUStack isn't even running), otherwise
    GPUStack. Route path kept stable so the dashboard poller needs no change."""
    env = current_app.config_manager.read_env()
    enabled = current_app.profile_manager.get_enabled_profiles()
    if 'llm-manager' in enabled:
        # Same call/module also pulls /api/workers so the panel can render the
        # inline GPU/VRAM readout a manager-only box lost with GPUStack. Both
        # calls degrade to ([], reason) on failure — the fragment never 500s.
        from app.services.llm_manager_client import get_served_models, get_workers
        main_domain = current_app.config.get('MAIN_DOMAIN', '')
        models, reason = get_served_models(env)
        workers, workers_reason = get_workers(env)
        return render_template('fragments/llm_manager_models.html',
                               models=models, reason=reason,
                               workers=workers, workers_reason=workers_reason,
                               main_domain=main_domain)
    from app.services.gpustack_client import get_models
    api_key = env.get('GPUSTACK_API_KEY', '')
...
def whats_new_history():
    ...
    # #524 The caveat this comment used to carry ("may need a fix at rc10+")
    # came due: the stack is at ga.12, and `sorted()` puts ga.9 above it, so the
    # newest What's New section was not the one shown first. Shared CalVer
    # comparator — unparseable names are kept but ordered last, so a stray file
    # is visible rather than silently promoted to the top.
    files = [version_to_path[v] for v in calver_sort(version_to_path, reverse=True)]
...
@api_bp.route('/api/apply/<action_id>')
def apply_status(action_id):
    """The CURRENT state of an apply action, without streaming (#1307).

    The apply itself already runs in a background thread; only the SSE stream
    below is a long-lived request — and on the Portal's single sync gunicorn
    worker a request that outlives `--timeout` is KILLED mid-flight. Measured on
    0.91 (2026-09-04): a `wazuh` enable streamed for five minutes, the worker
    was killed (`[CRITICAL] WORKER TIMEOUT (pid:7)`), and the client saw the
    stream end without a `done` event — while the action ran happily to the end
    (`Container wazuh-indexer Healthy` was in the stream). Every caller then had
    to GUESS, and the rotation guessed "failed" and ran a compensating disable
    on a module that had just come up.

    With this endpoint a broken stream is no longer a lost result: ask again.
    Deliberately cheap and non-streaming, so it answers inside any timeout.
    """
    action = current_app.apply_manager.get_action(action_id)
    if not action:
        return jsonify({'error': 'Action not found'}), 404
    lines, done, success, error, duration_ms = action.get_lines_from(0)
    return jsonify({
        'action_id': action_id,
        'done': bool(done),
