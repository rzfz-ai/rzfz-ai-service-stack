...
def _get_active_services(stack_root):
    ...
    try:
        result = subprocess.run(
            compose_argv(stack_root, 'config', '--services', logger=logger),
...
def _get_container_list():
    ...
    try:
        result = subprocess.run(
            compose_argv(stack_root, 'ps', '-a', '--format', 'json', logger=logger),
            capture_output=True, text=True, timeout=10,
            cwd=stack_root,
        )
        containers = []
        ...
        for c in containers:
            ...
            if not is_oneshot and '-init' in c.get('Name', '') and state == 'exited':
                is_oneshot = True
            # #1315 (rzfz review F1): "cleanly" has to mean it, or the label
            # HIDES the failure it was added to make visible. Before the
            # wazuh bootstraps were labelled, `wazuh-certs-generator` and
            # `wazuh-securityadmin` carried no `-init` in their names, so they
            # fell through this filter and an `Exited (1)` showed up in the
            # container matrix and counted against health — which is exactly
            # what #1259 needed. Labelling them without this exit-code test
            # would have dropped them silently.
            try:
                exit_code = int(c.get('ExitCode') or 0)
            except (TypeError, ValueError):
                exit_code = 0
            if is_oneshot and state == 'exited' and exit_code == 0:
                continue  # a one-shot that did its job is not degradation
...
def _build_dashboard_context():
    ...
    # Per-worker GPU/VRAM summary tiles (top resource row). On a box where the
    # LLM Manager is the active backend it fronts every worker; the inline
    # GPU/VRAM readout that used to come from GPUStack now comes from the
    # manager's /v1/workers (same read-only service-key path the LLM Models
    # panel uses in api.py — no new manager call, no new auth path). Only
    # queried when the profile is enabled, so non-manager boxes render the base
    # tiles unchanged and skip the network hop. get_workers() never raises: any
    # failure (no key, manager down, auth reject, junk body) degrades to
    # ([], reason), so the dashboard renders the base tiles and never 500s.
    llm_workers = []
    try:
        if 'llm-manager' in current_app.profile_manager.get_enabled_profiles():
            from app.services.llm_manager_client import get_workers
            env = current_app.config_manager.read_env()
            llm_workers, _ = get_workers(env)
    except Exception:
        llm_workers = []
