...
def detail(profile_id):
    ...
    # #1556 (Portal half): the acceptance probe's verdict, where the operator
    # who enabled the module actually looks. The execution path landed in
    # #1223/#1506 and the audit trail in the first half of this issue; the
    # Portal still queued a request, said "run `rzfz probe --pending`", and
    # never mentioned the answer again — so the verdict was only readable as a
    # JSON file on disk. Best-effort: a missing or unreadable queue must never
    # cost the operator the module page.
    probe = None
    try:
        from pathlib import Path as _Path
        from app.services import post_toggle_probe as _ptp
        probe = _ptp.latest_verdict_for_profile(
            profile_id, stack_root=_Path(current_app.config['STACK_ROOT']))
    ...
    return render_template('modules/detail.html',
                           ...
                           # #1443 rev-B (review): always-on hides the toggle only
                           # while the module is ENABLED — a box that predates the
                           # cutover must still be able to switch it on.
                           is_core=profile_id == 'core' or bool(profile.get('always_on') and is_enabled),
                           probe=probe,
