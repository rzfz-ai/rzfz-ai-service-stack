...
def take_checksum():
    ...
    # #1191 rev-B: the audit entry alone must say whether key-level .env
    # tracking was active for this set (env_key_id, the 12-hex id the detail
    # page shows) and which commit the tracked files were hashed against
    # (git_head). Both are None-pruned by the logger when not recorded, so
    # the detail text also spells the on/off state out.
    git_head = result.get('git_head')
    env_key_id = result.get('env_key_id')
    current_app.audit_logger.log(
        ...
        detail=(f'Checksum snapshot #{result["set_id"]}: {comment} '
                f'({result["file_count"]} files, {result["overall_sha256"][:12]}..., '
                f'git HEAD {(git_head or "not recorded")[:12]}, '
                f'.env key-level tracking {"on" if env_key_id else "off"})'),
        outcome='success',
        git_head=git_head,
