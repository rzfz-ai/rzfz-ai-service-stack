...
def clear_logs():
    ...
    # #1312 (BS-SECCOMP-BUG-03): truncating the operation log IS a mutation of
    # operator-visible state — and the one whose whole purpose is to remove a
    # record. Without an entry here, the act of clearing leaves no trace at
    # all, which is the single worst gap in an audit trail (R-SEC-11 /
    # R-CONFIG-03).
    user = session.get('admin_username', 'admin')
    source_ip = request.headers.get('X-Forwarded-For', request.remote_addr)
    # #1312 review: `outcome` was hardcoded to 'success' and `_docker_exec`'s
    # return code was thrown away, so a truncation that never happened (the
    # backup container down, the exec timing out) was recorded as one that did
    # — the three neighbouring routes in this file all form it as
    # `'success' if code == 0 else 'failure'`. Run first, then record what
    # happened: the earlier "log before acting so a logging failure cannot
    # swallow the act" only holds if the record is TRUE, and a false success is
    # worse than a missing entry.
    out, code = _docker_exec(BACKUP_CONTAINER, ['sh', '-c', f'> {LOG_FILE_REMOTE}'],
                             timeout=5)
    current_app.audit_logger.log(
        'backup.clear_logs', user=user, source_ip=source_ip,
        category='backup', action='clear_logs', target=LOG_FILE_REMOTE,
        detail=('Cleared the backup operation log' if code == 0 else
                f'FAILED to clear the backup operation log: {out.strip()[:200]}'),
        risk='caution', outcome='success' if code == 0 else 'failure',
    )
    if code != 0:
        return jsonify({'status': 'error', 'error': out.strip()[:400]}), 502
...
def settings():
    if request.method == 'POST':
        ...
        # Reconcile (never rebuild, #139) the offen exclude-regexp from BOTH
        # toggles. Model files (gpustack-data|speaches-data|llm-node-models|llm-registry-data) are excluded
        # unless the operator opts in; dify-plugin-daemon follows its own
        # toggle (#150); the #182 DB-datadir tokens are always enforced (they
        # are captured logically by core/backup/pre-backup.sh — removing them
        # requires restoring the stop labels in core/compose.yml, #157).
        # Every OTHER token the operator/runbook added survives the save.
        current_excl = current_app.config_manager.read_env().get(
            'BACKUP_EXCLUDE_REGEXP', '')
        new_excl = reconcile_exclude_regexp(
            current_excl, include_models, exclude_dify_plugins)
        ...
        updates = {
            ...
            'BACKUP_EXCLUDE_REGEXP': new_excl,
        }
