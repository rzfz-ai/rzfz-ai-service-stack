...
def send_support():
    ...
    if create_new or not filename:
        ...
        try:
            ...
            return jsonify({'success': False, 'message': f'Failed to create snapshot: {str(e)[:200]}'})
    else:
        filepath = os.path.join(SNAPSHOT_DIR, filename)

    if not os.path.exists(filepath):
        return jsonify({'success': False, 'message': 'Snapshot file not found.'})
    ...
    try:
        ...
        user = session.get('admin_username', 'admin')
        current_app.audit_logger.log(
            'logs.send_support', user=user,
            category='logs', action='send_support', target=email,
            detail=f'Sent {filename} to {email}',
            outcome='success',
        )
        return jsonify({'success': True, 'message': f'Log snapshot sent to {email}.'})

    except Exception as e:
        return jsonify({'success': False, 'message': f'Failed to send: {str(e)[:200]}'})
