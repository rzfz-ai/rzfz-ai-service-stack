...
already sits behind. `settings.<domain>` is gated by Caddy `forward_auth` →
...
# --- source-IP anchor (#377) -------------------------------------------------
# The SSO branch below trusts `X-Authentik-*` headers. Those are only meaningful
# because Caddy — the sole ingress, running Authentik `forward_auth` — sets them.
# Without an anchor, any container on the shared `_default` network could dial
# this app's port directly and forge
#
#     X-Authentik-Username: anyone
#     X-Authentik-Groups: razzfazz.ai Super Admins
#
# to become a super-admin of, by this module's own description, "the single most
# powerful UI in the stack" — /secrets, /secrets/regenerate, /auth, /domain,
# /smtp, module enable/disable, TLS, governance, backup/restore.
#
# The same class was already closed three times elsewhere (agent-manager's
# `_proxy_proof_ok`, start-portal's `proxy_anchor` #68, llm-manager's
# `from_caddy`) — the portal was missed by all three passes because the anchor
# had never been shared. It now is: `razzfazz_common.proxy_anchor` (#390).
_TRUST_ENV = "RZFZ_CONFIG_PORTAL_TRUST_ALL_PROXIES"
...
def register_auth_middleware(app):
    ...
    def require_admin_auth():
        ...
        # #377: only trust the headers if the request actually CAME from Caddy.
        # A peer that forges them without transiting Caddy is treated as if it
        # had sent no headers at all, so it falls through to the break-glass
        # branch and must present ADMIN_PASSWORD. We deliberately do NOT 403
        # here: that keeps the recoverability contract intact (a direct
        # host-port operator is exactly the break-glass case) while removing the
        # forgeable path to admin. Fails CLOSED — an unresolvable `caddy`
        # means "cannot verify", so the headers are not trusted.
        parsed = parse_authentik_headers()
        anchored = from_trusted_proxy(trust_env=_TRUST_ENV)
        if parsed['username'] and not anchored:
            current_app.logger.warning(
                "Ignoring X-Authentik-* headers on %s %s from %s — the request "
                "did not transit Caddy (#377). Falling back to break-glass auth.",
                request.method, request.path, request.remote_addr,
            )
        if parsed['username'] and anchored:
