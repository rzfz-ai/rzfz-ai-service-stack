...
from razzfazz_common.env_utils import (expand_env_refs, read_env_file,
                                        read_env_key)
...
def create_app():
    ...
    # CFG-13 (deliberately NOT applied here): the Start Portal sets
    # `MAX_CONTENT_LENGTH` because its largest legal body is a ~683 KB
    # base64 avatar. The Configuration Portal cannot: `settings.py`'s model
    # sideload streams MULTI-GIGABYTE .gguf uploads through
    # `request.files['model_file']`, so any global ceiling low enough to be
    # useful would break the feature. The sideload path has its own guards
    # (free-space check on the target volume, admin-only route, staged to a
    # temp dir), and this portal is admin-only behind Authentik — a body cap
    # belongs at the ingress (Caddy `request_body max_size` on the non-upload
    # routes), not here.
    ...
    # #1556: the return leg of the post-toggle probe. The host runner (`rzfz
    # probe --pending`, #1223/#1506) writes its verdict next to the request as
    # JSON and cannot reach the audit trail — that lives in a named volume and
    # this app is the tree's only writer to it. So the Portal carries every
    # verdict over as its own `probe.result` entry, on a tick rather than on a
    # page render: a trail that only fills when somebody opens the UI is not
    # one. Started here, next to the other poll loop, so the two are found
    # together.
    from app.services.post_toggle_probe import ProbeVerdictRecorder
    app.probe_verdict_recorder = ProbeVerdictRecorder(
        app.audit_logger, app.config['STACK_ROOT'])
    app.probe_verdict_recorder.start()
    ...
    # CFG-1 (#1018) — app-wide CSRF, the same way the Start Portal turned it
    # on in #388. Before this, the Configuration Portal — by auth.py's own
    # description "the single most powerful UI in the stack" — had NO CSRF
    # anywhere: every state-changing route (secrets regenerate, .env writes,
    # profile toggles, factory reset, backup restore, /api/apply) was
    # cross-site forgeable from any page the admin's browser visited while
    # holding a portal session or an Authentik SSO cookie.
    #
    # Registered AFTER register_auth_middleware on purpose: Flask runs
    # before_request hooks in registration order and stops at the first one
    # that returns a response, so an UNauthenticated POST still gets the
    # login redirect it always got (no behaviour change for the break-glass
    # flow), and the CSRF hook only sees requests that already passed the
    # auth gate. Fail-closed for every route, including ones added later —
    # which is exactly the failure mode #1018 describes (new state-changing
    # routes shipping without protection).
    #
    # Client side: base.html puts the token in `<meta name="csrf-token">`
    # (read by static/main.js for its fetch() calls + per-page inline JS)
    # and on `<body hx-headers>` so every HTMX request inherits the
    # `X-CSRF-Token` header. Plain <form method="POST"> carries the hidden
    # `_csrf_token` field.
    #
    # Nothing POSTs to this container service-to-service (it is a
    # browser-only admin UI; the day1 probes only GET), so there is no
    # server-side caller to exempt. /healthz is exempt inside
    # razzfazz_common.csrf.
    ...
    # #1876 / #1200 — the Portal's OWN address is SETTINGS_DOMAIN. `CONFIG_DOMAIN`
    # is the retired host, kept one cycle as a 301 onto this one
    # (config/.env.example: "Point nothing new at CONFIG_DOMAIN"), so a surface
    # that prints `config.<domain>` advertises the name the operator is being
    # migrated OFF.
    #
    # READ the key, never rebuild `settings.<MAIN_DOMAIN>` from MAIN_DOMAIN:
    # SETTINGS_DOMAIN is an operator-settable value and #1200 explicitly allows
    # a name off the template (`SETTINGS_DOMAIN=portal.acme.com` — upgrade.sh's
    # pre-flight only forbids it COLLIDING with CONFIG_DOMAIN). A reconstructed
    # string would print an address the box does not serve.
    #
    # Expanded because the shipped default is a template
    # (`settings.${MAIN_DOMAIN}`) and .env carries it verbatim on boxes whose
    # writer didn't expand it — but expanded against THE FILE, not the process
    # env: this container also carries MAIN_DOMAIN as an env var (compose), and
    # where the two disagree the file is what the box is configured from, which
    # is why MAIN_DOMAIN above is read from the file too. Falls back to the
    # documented default for boxes that predate the #1200 migration and have no
    # key at all.
    env_file = read_env_file(env_path)
    settings_domain = expand_env_refs(env_file.get('SETTINGS_DOMAIN', ''), env_file)
    if not settings_domain and main_domain:
        settings_domain = f'settings.{main_domain}'
    if settings_domain:
        app.config['SETTINGS_DOMAIN'] = settings_domain
...
def _nav_extras(app):
    ...
    # #1189: is the Portal's single-file .env bind still the host's file?
    # Two stat/readdir probes per request; a failing probe hides the banner
    # rather than the page.
    try:
        from app.services import env_mount as _env_mount
        env_mount_state = _env_mount.inspect_env_mounts(stack_root)
    except Exception:
        env_mount_state = None
    ...
    return {
        ...
        # #1876: the Portal's own address, resolved once at startup from
        # SETTINGS_DOMAIN (create_app). Empty on a box with neither key — the
        # sidebar then falls back to a generic label rather than inventing a
        # hostname.
        'settings_domain': app.config.get('SETTINGS_DOMAIN', ''),
