...
def get_gpu_devices(api_key):
    ...
    if not api_key:
        return []
    # #472: GPUStack 0.7.x serves /v1/*, 2.x serves /v2/*. Every other reader
    # in this file already walks both (`_get_running_instances`,
    # `_get_running_model_records`); this one asked /v1 only, so on a 2.x box
    # it 404'd and the `except` turned that into "no GPUs" — the dashboard's
    # own empty-list path then rendered the host card alone, without an error.
    # Nothing to notice: the panel that should list GPUs simply has none.
    data = None
    for path in ('/v1/gpu-devices', '/v2/gpu-devices'):
        data = _get_json(api_key, path)
        if data is not None:
            break
    if data is None:
