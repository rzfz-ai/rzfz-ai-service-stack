    ...
    def _fetch_via_url(self, url):
        ...
        try:
            ...
            if ctype and ctype not in ('application/json', 'text/plain', 'text/json'):
                ...
                # #275 ask 3: the advice has to match the channel. Telling a
                # PUBLIC box to point origin at git.razzfazz.ai sends the
                # operator at the one host that box cannot reach — which is
                # the whole reason the public path exists.
                if self._channel() == 'public':
                    hint = (
                        'On a public/customer box this URL is the GitHub raw '
                        'mirror and needs no credentials, so a non-JSON reply '
                        'means egress is being intercepted: check the '
                        'corporate proxy (HTTPS_PROXY) and that its CA is '
                        'trusted by the config container. Do NOT point origin '
                        'at git.razzfazz.ai — it is unreachable from here.'
                    )
                else:
                    hint = (
                        'Workaround: ensure the box has git origin set to '
                        'git.razzfazz.ai (the smart-HTTP path is exempt from '
                        'the proxy and image_checker prefers it when available).'
                    )
                return False, (
