...
import hardware_catalog  # noqa: E402  — #2158: per-hardware auto_start + defaults
import expected_models  # noqa: E402  — shared mmproj-sidecar derivation (#138)
...
def _defaults(spec):
    """#2158: `defaults` with the box's hardware-family overrides applied
    (`defaults_by_hardware`), the box's class read from .env HARDWARE — or,
    #2460, the cluster family post-install resolved for a CPU master that
    serves through GPU workers."""
    return hardware_catalog.defaults_for(spec, hardware_catalog.catalog_hardware(ENV_FILE))
...
_PSQL_VAR_RE = re.compile(r"\A[A-Za-z_][A-Za-z0-9_]*\Z")


def postgres_exec(database: str, sql: str,
                  params: dict[str, str] | None = None) -> tuple[str, str]:
    """Run `sql` in `database` via `psql -tAc`, with optional bound variables.

    #378: identifiers read back OUT of a database were previously f-string
    interpolated into the SQL executed here — and this runs as `-U docker`, the
    cluster SUPERUSER (POSTGRES_USER, the role init-db.sh uses to CREATE ROLE),
    while `psql -c` happily accepts several `;`-separated statements. A model id
    containing a quote was therefore a second-order SQL injection with superuser
    reach.

    `params` are passed to psql as `-v name=value` and referenced in `sql` as
    `:'name'`, which psql expands as a properly quoted SQL literal (embedded
    single quotes doubled). Quoting is done by psql, not by us — the bug this
    replaces was hand-rolled `.replace("'", "''")` applied to the params VALUE
    while the identifier in the same statement's WHERE clause got nothing.

    Values are argv elements (`run()` uses a list, shell=False), so there is no
    shell-quoting layer to get wrong either.
    """
    args = ["docker", "exec", "postgres", "psql", "-U", "docker", "-d", database]
    for name, value in (params or {}).items():
        # psql variable names are identifiers; reject anything else outright
        # rather than emit a `-v` psql would parse in some other way.
        if not _PSQL_VAR_RE.match(name):
...
def sync_gpustack(spec: dict, dry_run: bool, verbose: bool) -> list[str]:
    ...
    for name, model in spec["models"].items():
        ...
        # #138: offline vision — point --mmproj at the sideloaded local-models
        # copy, exactly like post-install's first registration. Without this
        # the reconcile would flag the rewritten param as drift and revert it
        # (back to the nonexistent HF-cache path) on every sync run.
        sidecar = expected_models.mmproj_filename(model)
        if sidecar:
            desired_params = model_source.apply_mmproj(
                desired_params, model["huggingface_repo_id"], sidecar,
...
def sync_openwebui(spec: dict, dry_run: bool, verbose: bool) -> list[str]:
    ...
    for stale in set(existing) - desired_ids:
        ...
        out2, _ = postgres_exec(
            "openwebui_db", "SELECT base_model_id FROM model WHERE id = :'ident';",
            {"ident": stale})
        if out2.strip():
            if verbose:
                print(f"  openwebui: skipping custom model {stale} (base_model_id set)")
            continue
        # #1305: a row stamped `meta.managed_by = "llm-manager"` belongs to
        # scripts/owui_model_rows.py — it holds a model the MANAGER serves (a
        # console deployment is never a YAML key) and that writer removes it
        # when the manager stops serving it. Deleting it here re-opened the
        # pendulum: sync deletes, the writer re-creates, CHANGED never hits 0.
        out3, _ = postgres_exec(
            "openwebui_db",
            "SELECT (meta::json)->>'managed_by' FROM model WHERE id = :'ident';",
            {"ident": stale})
        if out3.strip() == "llm-manager":
            if verbose:
                print(f"  openwebui: keeping {stale} (row owned by the LLM Manager "
                      f"row writer, not by standard-models.yaml — #1305)")
            continue
        changes.append(f"openwebui: DELETE {stale}")
        if not dry_run:
            postgres_exec("openwebui_db", "DELETE FROM model WHERE id = :'ident';",
                          {"ident": stale})
    ...
    for name, model in spec["models"].items():
        ...
        out, _ = postgres_exec(
            "openwebui_db", "SELECT params FROM model WHERE id = :'ident';",
            {"ident": name})
        if not out.strip():
            # #1266: there is no "OWUI auto-discovery" that creates this row,
            # and deferring to one is why a Manager box ended up with an EMPTY
            # `model` table. The row is WRITTEN — by the `model-sync` container
            # on a GPUStack box, by post-install's OWUI step
            # (scripts/owui_model_rows.py) on a Manager box — and that writer
            # puts these very params on the row at INSERT time, because in a
            # fresh provision this reconcile runs BEFORE it (post-install step
            # 4b vs step 5). So a missing row here is expected on a first run
            # and a genuine defect on a second.
            changes.append(
                f"openwebui: SKIP {name} (no `model` row yet — the Open WebUI "
                f"provisioning step writes the rows and their context params; "
                f"if it is still missing after that, re-run "
                f"'rzfz post-install --refresh')")
            continue
        try:
        ...
        if not dry_run:
            ...
            postgres_exec(
                "openwebui_db",
                "UPDATE model SET params = :'params'::json WHERE id = :'ident';",
                {"params": json.dumps(new_params), "ident": name})
    ...
    for name in stale:
        ...
        if not dry_run:
            postgres_exec(
                ...
                "WHERE provider_name = 'langgenius/gpustack/gpustack' "
                "AND model_name = :'ident';",
                {"ident": name},
            )
...
def sync_cognee(spec: dict, dry_run: bool, verbose: bool) -> list[str]:
    ...
    changes: list[str] = []
    chat_default = _defaults(spec).get("chat")
    embed_default = _defaults(spec).get("embedding")
...
def sync_hermes(spec: dict, dry_run: bool, verbose: bool) -> list[str]:
    ...
    model_name = _defaults(spec)[role]
...
def sync_openhands(spec: dict, dry_run: bool, verbose: bool) -> list[str]:
    ...
    chat_alias = _defaults(spec).get(role, "qwen3.6")
    embed_alias = _defaults(spec).get("embedding", "nomic-embed-text")
