...
def apply_mmproj(params: list, repo: str, mmproj_filename: str,
                 mode: str = "online") -> list:
    """Adjust a model's ``--mmproj`` backend parameter for the box's network
    mode (#138 — offline vision).

    Online, GPUStack resolves the sidecar itself: either the declared
    ``--mmproj=<HF-cache path>`` (granite-docling) or repo auto-detection
    (qwen3.6). Offline, neither works — the HF cache never exists and
    ``source=local_path`` has no auto-detect — so registration must point at
    the copy the offline package staged under
    ``local-models/<repo>/<mmproj_filename>``.

    ``mode``:
      * ``"online"``  — return ``params`` unchanged.
      * ``"offline"`` — drop any existing ``--mmproj=…`` and append
        ``--mmproj=<local-models path>``.
      * ``"strip"``   — drop any ``--mmproj=…`` and add nothing: the sidecar is
        not sideloaded, and llama-box aborts on a nonexistent ``--mmproj`` path
        — text-only beats no model at all.
    """
    if mode == "online":
        return list(params)
    kept = [p for p in params if not str(p).startswith("--mmproj=")]
    if mode == "offline":
        kept.append("--mmproj=" + local_model_path(
            f"{repo}/{mmproj_filename}"))
    return kept
