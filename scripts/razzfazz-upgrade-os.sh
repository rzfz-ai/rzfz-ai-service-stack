...
phase_reconcile() {
    ...
    local newest_signed
    # #1318: on a box (or a CI container) without kernel images under /boot the
    # `ls` fails (rc 2) and, under `set -euo pipefail`, took the whole script
    # down silently right here — the held-kernel test ended after 18 checks
    # with RC 2 and no verdict. No image is an answer ("none found"), not a crash.
    newest_signed="$({ ls -1 /boot/vmlinuz-*-generic 2>/dev/null || true; } | sed 's#.*/vmlinuz-##' | sort -V | tail -1)"
    ...
    print_success "Reconciliation complete. Verify the UIs (settings.<domain>, chat.<domain>, …)."
}
