...
try:
    ...
    for m in data.get('items', []):
        ...
        if 'embedding' in cats:
            desc = 'Embedding-Modell zur Vektorisierung von Texten. Wird für semantische Suche, RAG-Pipelines und Dokumentenähnlichkeit in Dify und Open WebUI eingesetzt.'
        elif 'reranker' in cats:
            desc = 'Reranking-Modell zur Verbesserung von Suchergebnissen. Sortiert Kandidaten nach Relevanz und steigert die Qualität von RAG-Antworten.'
        elif 'gemma' in name_l:
            desc = 'Googles Gemma-Sprachmodell für Chat, Textgenerierung und Instruktionsbefolgung. Gut geeignet für allgemeine Frage-Antwort-Szenarien und kreative Textarbeit.'
        elif 'magistral' in name_l:
            desc = 'Mistrals Magistral-Modell mit starker Reasoning-Fähigkeit. Besonders geeignet für analytische Aufgaben, Code-Generierung und komplexe Instruktionen.'
        elif 'qwen' in name_l and 'embedding' not in cats and 'reranker' not in cats:
            desc = 'Alibabas Qwen-Sprachmodell mit Mixture-of-Experts-Architektur. Effizient bei hoher Parameteranzahl, geeignet für Chat, Analyse und mehrsprachige Aufgaben.'
        else:
            desc = 'Allgemeines Sprachmodell für Chat und Textgenerierung.'
        ...
        rows.append(f'<tr><td colspan=\"5\" style=\"font-size:8.5pt;color:#555;border-top:none;padding-top:0;\">{desc}</td></tr>')
    if rows:
        print(''.join(rows))
except Exception:
    pass
