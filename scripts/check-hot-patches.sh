...
# Space-separated list of boxes an operator has reconciled OUT OF BAND and
# attests are clean — for boxes this script cannot reach with a passwordless key
# (the production box is OpenVPN-only with password auth; its `prod-vm` alias is
# not key-reachable from a headless release run). Reconcile it directly and
# record the box here, e.g.
#   RAZZFAZZ_HOTPATCH_ATTESTED="prod-vm" ./scripts/prepare-release.sh <tag> --check
# The attestation is logged and the box is treated as reconciled, NOT as drift.
ATTESTED="${RAZZFAZZ_HOTPATCH_ATTESTED:-}"

ISSUES=0        # real drift: a live patch not in source HEAD  -> exit 1
UNREACHABLE=0   # could not check (SSH/setup)                   -> exit 2 (distinct)
...
for box in $FLEET; do
    ...
    # Operator-attested: reconciled out of band (e.g. the OpenVPN-only prod box,
    # not key-reachable from a headless run). Treat as clean, not drift.
    case " $ATTESTED " in
        *" $box "*)
            echo -e "${GREEN}  [✓] $box operator-attested reconciled (RAZZFAZZ_HOTPATCH_ATTESTED) — not re-checked here.${NC}"
            continue
            ;;
    esac

    if ! ssh -o ConnectTimeout=8 -o BatchMode=yes "$box" 'true' 2>/dev/null; then
        # Could-not-check is NOT the same as found-drift. A key-unreachable box
        # (prod is OpenVPN + password) must not silently read as "a patch was
        # found." Count it separately; the operator reconciles it directly and
        # re-runs with RAZZFAZZ_HOTPATCH_ATTESTED="$box".
        echo -e "${YELLOW}  [!] $box unreachable via passwordless SSH.${NC}"
        echo -e "${YELLOW}      Reconcile it directly (prod is OpenVPN + password), then re-run with:${NC}"
        echo -e "${YELLOW}      RAZZFAZZ_HOTPATCH_ATTESTED=\"$box\" ...${NC}"
...
echo
if [ "$ISSUES" -gt 0 ]; then
    echo -e "${RED}[✗] Hot-patch reconciliation: $ISSUES real drift(s) detected.${NC}"
    echo -e "${YELLOW}    Either commit the patches to source or remove them from the affected box before tagging.${NC}"
    exit 1
fi

if [ "$UNREACHABLE" -gt 0 ]; then
    echo -e "${YELLOW}[!] Hot-patch reconciliation: no drift found, but $UNREACHABLE box(es) could not be checked over passwordless SSH.${NC}"
    echo -e "${YELLOW}    Reconcile them directly and re-run with RAZZFAZZ_HOTPATCH_ATTESTED=\"<box> ...\".${NC}"
    exit 2
fi
