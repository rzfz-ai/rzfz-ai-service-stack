...
api_set_password() {
    ...
    # 1. Find akadmin's pk (Super-Admin scope; -k tolerates self-signed
    #    certs on TLS_MODE=internal stacks).
    local pk_json pk
    pk_json=$(curl -sk --max-time 30 \
        -H "Authorization: Bearer ${AUTHENTIK_BOOTSTRAP_TOKEN}" \
        -H "Accept: application/json" \
        "${base}/api/v3/core/users/?username=akadmin" || true)
    if [[ -z "$pk_json" ]]; then
    ...
    if [[ -z "$pk" ]]; then
        print_error "rotate-bootstrap-password: akadmin not found via ${base}/api/v3/core/users/?username=akadmin"
        print_info  "  Token may be invalid or akadmin renamed."
        return 1
    fi
...
#    against the OLD bootstrap password after a rotation (config.<domain> login
