...
#   - Locally-BUILT services are NEVER rewritten — their image is built on the
#     box, not pulled, so a mirror ref would be wrong. Two signals say so, and on
#     an installed box only the second survives: `build:` (present in the build
#     tree) and `pull_policy: never` (#1549 — `compose.no-build.yml` strips
#     `build:` on every installed box, which is exactly the chain this generator
#     discovers through). Only pinned-image (pulled) services get the rewrite.
...
def rewrite_services(services, mirror):
    ...
    ``services`` maps ``service -> {'image': str, 'build': …, 'pull_policy': …}``
    (the shape ``docker compose config --format json`` emits). A service is
    skipped when it is LOCALLY BUILT or has no ``image:``. The remaining
    services get their ``image:`` mirror-rewritten; ``pull_policy`` is
    deliberately NOT touched.

    #1549 — "locally built" is TWO questions, and `build:` alone answers the
    wrong one on the boxes this overlay actually runs on. `compose.no-build.yml`
    (#184 WS2a) sets ``build: !reset null`` on every custom-build service, and
    it is in the COMPOSE_FILE chain of every installed box — the same chain this
    generator discovers services through. So on an installed box a locally built
    service arrives here with no ``build`` key and looks pulled. Measured on the
    DevBox:

        COMPOSE_FILE="compose.yml"                       gpustack-legacy build: yes
        COMPOSE_FILE="compose.yml:compose.no-build.yml"  gpustack-legacy build: NO

    and because the overlay is APPENDED to the chain (``compose_file_overlay_add``),
    its ``image:`` then wins over the one the no-build overlay just set. With
    ``pull_policy: never`` still in force, compose looks for a LOCAL image under
    the mirror name that nobody ever creates, and the service does not start —
    on the one box shape (registry mirror + air-gap) where recovering is hardest.

    ``pull_policy: never`` is therefore the second, and on an installed box the
    only surviving, signal: a service that by definition never pulls has no
    business in a pull-redirection overlay. Both checks stay, because the
    build-tree chain (no `compose.no-build.yml`) still answers with ``build:``
    and a future service might carry one without the policy.
    """
    out = {}
    for name, svc in services.items():
        ...
        if str(svc.get('pull_policy') or '').strip().lower() == 'never':
            continue  # #1549: never pulls → redirecting its pull is meaningless
        image = svc.get('image')
        if not image:
            continue
