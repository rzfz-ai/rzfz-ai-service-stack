...
harden_docker_daemon() {
    ...
    if [[ -f "$daemon_json" ]]; then
        # File exists -- check if it already has our settings
        if grep -q '"no-new-privileges"' "$daemon_json" 2>/dev/null; then
            ok "$daemon_json already contains hardening settings -- skipping"
            return
        fi
        warn "$daemon_json exists but missing hardening settings."
        warn "Please merge the following settings manually or review docs/daemon.json.example:"
        warn "  log-driver, log-opts, live-restore, no-new-privileges, userland-proxy, storage-driver"
        return
    fi

    mkdir -p /etc/docker

    # NOTE: previously included `"userland-proxy": false`. Removed 2026-05-10
    # after prod 2026.05 cutover surfaced a Docker bridge DNS race (containers
    # transiently see `lookup <other-svc>: server misbehaving` from 127.0.0.11
    # during cascading recreates). With userland-proxy disabled, the embedded
    # DNS goes through pure iptables NAT — there's a documented race window
    # while iptables rules are rewritten. Default (`true`) uses a small per-port
    # proxy process that handles connection setup more resiliently. The perf
    # cost is negligible on a private bridge network. See
    # .gsd/reports/morning-report-2026-05-10.md for the full diagnosis.
    cat > "$daemon_json" <<'EOF'
{
