...
# Daemon/host-level NO_PROXY. This value lands in the systemd drop-in AND in
# ~/.docker/config.json proxies.default — the latter is inherited by EVERY
# container (not just the overlaid egress ones). So it MUST list every internal
# service/container hostname, else a non-egress service that makes an internal
# call (e.g. openwebui→gpustack:9090) routes it through the proxy and fails —
# httpx matches NO_PROXY by EXACT host, never the CIDR ranges (#280). We reuse
# the overlay generator as the single source of truth (all profiles). Registry
# pulls still go through the proxy — we exclude only same-box/internal traffic.
_compute_no_proxy() {
    local md gen np; md="$(_env_get MAIN_DOMAIN)"
    gen="$(python3 "${SCRIPT_DIR}/scripts/gen-corporate-proxy-overlay.py" \
              --print-no-proxy --proxy-url "$PROXY_URL" \
              ${md:+--main-domain "$md"} \
              ${NO_PROXY_EXTRA:+--no-proxy "$NO_PROXY_EXTRA"} 2>/dev/null)"
    if [ -n "$gen" ]; then
        printf '%s' "$gen"
        return 0
    fi
    # Fallback (generator/docker unavailable): lean CIDR list + loopback +
    # host.docker.internal. Less complete than the enumerated list, but safe.
    np="localhost,127.0.0.1,::1,0.0.0.0,host.docker.internal,172.16.0.0/12,10.0.0.0/8,192.168.0.0/16,.internal,.svc,.local"
    [ -n "$md" ] && np="${np},${md},.${md}"
...
# #277: render the box-local squid.conf for the dify-ssrf-chain sidecar. The
# sidecar chains Dify's PUBLIC egress (marketplace.dify.ai plugin downloads)
# through the corporate WSA while keeping the doc-tools direct + blocking other
# internal targets — caddy:8195 cannot chain to a plain-HTTP upstream. The
# rendered conf lands box-local under certs/ (gitignored) and is mounted RO into
# the sidecar by the overlay (--squid-conf-host-path default ./certs/...).
_render_squid_conf() {
    local tmpl="modules/dify/ssrf-chain/squid.conf.tmpl"
    local out="certs/dify-ssrf-chain-squid.conf"
    [ -f "$tmpl" ] || { print_warning "squid template $tmpl missing — dify-ssrf-chain not rendered (#277)"; return 1; }
    # WSA host + port from the proxy URL (strip scheme + creds; default :3128).
    local hp h p; hp="${PROXY_URL#*://}"; hp="${hp#*@}"
    h="${hp%%[:/]*}"; p="${hp##*:}"; p="${p%%/*}"
    [ "$p" = "$h" ] && p=3128
    # Doc-tools allow-list (space-separated) — internal AI tools Dify may reach
    # DIRECTLY (never via the WSA). Default mirrors config/.env.example.
    local doctools; doctools="$(_env_get DIFY_DOC_TOOLS_SSRF_ALLOW)"
    [ -n "$doctools" ] || doctools="gpustack ollama-proxy speaches searxng crawl4ai docling tika presidio-analyzer presidio-anonymizer presidio-image-redactor stirling-pdf gotenberg lightrag cognee cognee-mcp gitea"
    mkdir -p certs
    sed -e "s#@@WSA_HOST@@#${h}#g" \
        -e "s#@@WSA_PORT@@#${p}#g" \
        -e "s#@@DOCTOOLS_DOMAINS@@#${doctools}#g" \
        "$tmpl" > "$out"
}
...
if [ "$MODE" = "regenerate" ]; then
    # #277/#280/#275: Level-C-only refresh of the box-local overlay + Dify
    # SSRF-chain squid.conf from the CURRENT generator, using values already
    # captured in .env. NO host/sudo changes, NO reachability gate — safe to call
    # non-interactively from `rzfz upgrade` so overlay-side fixes (comprehensive
    # NO_PROXY, the Dify marketplace chaining sidecar, the razzfazz-config CA)
    # land on a proxied box WITHOUT a manual `rzfz setup --corporate-proxy` re-run.
    if [ "$(_env_get RAZZFAZZ_CORPORATE_PROXY)" != "1" ]; then
        print_substep "corporate-proxy not enabled — nothing to regenerate."
        exit 0
    fi
    PROXY_URL="$(_env_get RAZZFAZZ_PROXY_URL)"
    NO_PROXY_EXTRA="$(_env_get RAZZFAZZ_PROXY_NOPROXY)"
    PROXY_HOST_PIN="$(_env_get RAZZFAZZ_PROXY_HOST_PIN)"
    EGRESS_SET="$(_env_get RAZZFAZZ_PROXY_EGRESS_SET)"; [ -n "$EGRESS_SET" ] || EGRESS_SET="standard"
    MAIN_DOMAIN="$(_env_get MAIN_DOMAIN)"
    if [ -z "$PROXY_URL" ]; then
        print_warning "RAZZFAZZ_CORPORATE_PROXY=1 but RAZZFAZZ_PROXY_URL unset — cannot regenerate overlay. Run 'rzfz setup --corporate-proxy --proxy-url … --ca-file …'."
        exit 0
    fi
    print_step "Regenerating ${OVERLAY_FILE} (Level C only)"
    _render_squid_conf && print_substep "Rendered certs/dify-ssrf-chain-squid.conf (#277 Dify marketplace chain)."
    _gen_args=( --proxy-url "$PROXY_URL" --egress-set "$EGRESS_SET" --dify-ssrf-chain --out "$OVERLAY_FILE" )
    ...
    if python3 "${SCRIPT_DIR}/scripts/gen-corporate-proxy-overlay.py" "${_gen_args[@]}"; then
        compose_file_add_overlay
        print_success "Regenerated ${OVERLAY_FILE} (comprehensive NO_PROXY + Dify SSRF chain)."
    else
        print_error "Overlay regeneration failed."
        exit 1
    fi
    exit 0
fi
...
# --dify-ssrf-chain (#277): emit the marketplace chaining sidecar. Harmless when
# Dify is not enabled — the sidecar carries profiles:[dify] so it only starts
# with the Dify profile; the generator gates emission on Dify being a service.
_gen_args=( --proxy-url "$PROXY_URL" --egress-set "$EGRESS_SET" --dify-ssrf-chain --out "$OVERLAY_FILE" )
[ -n "$MAIN_DOMAIN" ]    && _gen_args+=( --main-domain "$MAIN_DOMAIN" )
[ -n "$NO_PROXY_EXTRA" ] && _gen_args+=( --no-proxy "$NO_PROXY_EXTRA" )
[ -n "$PROXY_HOST_PIN" ] && _gen_args+=( --proxy-host-pin "$PROXY_HOST_PIN" )
if [ "$MODE" = "apply" ]; then
    ...
    print_substep "${DRY}would render certs/dify-ssrf-chain-squid.conf (#277 Dify marketplace chain)"
