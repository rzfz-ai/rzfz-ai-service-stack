...
#                          (exit 1 = tooling failure, e.g. no images enumerated)
...
# syft extracts every layer into TMPDIR. On 0.91 /tmp is a 16 GB tmpfs: the SEVEN largest
# images (23, 12.5, 10, 7.9, 6.6, 5.7 GB — the ones with the most packages and CVEs) failed
# with "disk quota exceeded" and the run still exited 0 with 45 of 52 images (2026-09-20).
# Default TMPDIR to the output filesystem unless the caller set one; cleaned up on exit.
SBOM_OWN_TMPDIR=""
if [ -z "${TMPDIR:-}" ]; then
    export TMPDIR="$OUT_DIR/.tmp"
    SBOM_OWN_TMPDIR="$TMPDIR"
    mkdir -p "$TMPDIR"
fi
_sbom_cleanup_tmpdir() { [ -n "${SBOM_OWN_TMPDIR:-}" ] && rm -rf "$SBOM_OWN_TMPDIR"; return 0; }
...
_sbom_manifest_pins() {  # every declared third-party pin as image:current, one per line
    [ -f "$STACK_DIR/config/manifests/versions.json" ] && command -v python3 >/dev/null 2>&1 || return 0
    python3 -c "
import json, sys
try:
    m = json.load(open('$STACK_DIR/config/manifests/versions.json'))
    for sect in ('images', 'hardcoded'):
        for _, v in m.get(sect, {}).items():
            if v.get('image') and v.get('current'):
                print(f\"{v['image']}:{v['current']}\")
except Exception as e:
    sys.stderr.write(f'manifest enumeration failed: {e}\n')
" 2>/dev/null || true
}
...
enumerate_images() {
    ...
    local compose_imgs="" running_imgs="" runtime_only_imgs="" catalog_imgs="" manifest_imgs=""
    if command -v docker >/dev/null 2>&1; then
        if [ -f "$STACK_DIR/compose.yml" ]; then
            # #2331: `docker compose config` honours COMPOSE_PROFILES, so on the GA
            # build box it listed only the 12 enabled modules and the artefact
            # described the box, not the release (32 of 63 declared pins never
            # scanned; the diff read every disabled module's CVEs as GONE).
            # `--profile '*'` enables every profile for the config render only.
            compose_imgs="$(cd "$STACK_DIR" && docker compose --profile '*' config --images 2>/dev/null || true)"
            [ -n "$compose_imgs" ] || compose_imgs="$(cd "$STACK_DIR" && docker compose config --images 2>/dev/null || true)"
        fi
...
" 2>/dev/null || true)"
    ...
    # (e) #2331: EVERY declared third-party pin in config/manifests/versions.json
    #     (images + hardcoded, image:current) — the tracked source that does not
    #     depend on which profiles a box has on. A release artefact derives from
    #     tracked sources (#491); the compose render and the catalog add what the
    #     manifest cannot express (our own builds, per-user agents).
    manifest_imgs="$(_sbom_manifest_pins)"
...
" 2>/dev/null || true)"
    ...
    # #491: the published SBOM is built from TRACKED sources only (compose
    # config, manifest runtime_only pins, agent catalog). `docker ps` output
    # is the state of one machine at one moment — a hand-built image on the
    # packaging box ended up in a published, customer-facing SBOM as though
    # it were part of the product (razzfazz-llama-vulkan-runner:latest,
    # reproducible by nobody). Running images are now only CROSS-CHECKED:
    # anything running that no tracked source explains is warned about loudly
    # and excluded from the artifact.
    local tracked
    # #2315: the sort and the comm MUST share one collation. Under a UTF-8 locale GNU sort
    # gives '-' and '_' no primary weight, comm then sees "not in sorted order", exits 1,
    # and with set -e the whole enumeration died before printing the tracked list — the
    # script reported "No images to scan" with exit 2, the code reserved for NEW
    # CRITICAL/HIGH findings. LC_ALL=C on both sides, and the cross-check (a WARNING)
    # can never abort the enumeration again.
    tracked="$(printf '%s\n%s\n%s\n%s\n' "$compose_imgs" "$runtime_only_imgs" "$catalog_imgs" "$manifest_imgs" | sed '/^\s*$/d' | LC_ALL=C sort -u)"
    echo "enumeration: manifest pins $(printf '%s\n' "$manifest_imgs" | sed '/^\s*$/d' | wc -l | tr -d ' '), compose (all profiles) $(printf '%s\n' "$compose_imgs" | sed '/^\s*$/d' | LC_ALL=C sort -u | wc -l | tr -d ' '), tracked union $(printf '%s\n' "$tracked" | sed '/^\s*$/d' | wc -l | tr -d ' ')" >&2
    local untracked
    untracked="$(LC_ALL=C comm -23 <(printf '%s\n' "$running_imgs" | sed '/^\s*$/d' | LC_ALL=C sort -u) <(printf '%s\n' "$tracked") 2>/dev/null || true)"
    if [ -n "$untracked" ]; then
        {
            echo "WARN (#491): images RUNNING on this box that NO tracked source (compose/manifest/catalog) explains —"
            echo "             excluded from the SBOM; they are machine-local state, not part of the release:"
            printf '             %s\n' $untracked
        } >&2
    fi
    printf '%s\n' "$tracked"
...
# #2331 control, computed HERE (enumerate_images runs in a subshell): the declared pins
# are the floor of a release artefact; manifest.json records declared vs enumerated.
ENUM_DECLARED_PINS="$([ -n "$IMAGES_FROM" ] && echo 0 || _sbom_manifest_pins | sed '/^\s*$/d' | LC_ALL=C sort -u | wc -l | tr -d ' ')"
ENUM_TRACKED="${#IMAGES[@]}"
if [ "${#IMAGES[@]}" -eq 0 ]; then
    # Exit 1, not 2: an empty image set is a TOOLING failure (docker missing, a compose
    # file that does not resolve, an empty --images-from), never a security finding.
    # Exit 2 stays reserved for NEW CRITICAL/HIGH findings so the release gate is
    # mechanical (#2315: both used to be 2 and were indistinguishable from outside).
    print_error "No images to scan — enumeration produced nothing (tooling failure, not a finding)."
    exit 1
fi
...
for img in "${IMAGES[@]}"; do
    print_substep "$img"
    if ! scan_one_image "$img" "$IMAGES_DIR"; then
        FAILED_IMAGES+=("$img")
    fi
done
...
# A failed scan is a NAMED gap, never silence: a reader of releases/<tag>/sbom/ must be able
# to tell "no CVEs" from "never scanned", and a diff must not turn a scanner failure on one
# side into phantom GONE rows. Any failure makes the run exit 1 (tooling) at the end.
SCAN_FAILED=0
: > "$OUT_DIR/failed-images.txt"
if [ "${#FAILED_IMAGES[@]}" -gt 0 ]; then
    SCAN_FAILED=1
    printf '%s\n' "${FAILED_IMAGES[@]}" > "$OUT_DIR/failed-images.txt"
    print_error "${#FAILED_IMAGES[@]} of $TOTAL_IMAGES image(s) were NOT scanned (see $OUT_DIR/failed-images.txt):"
    printf '    %s\n' "${FAILED_IMAGES[@]}" >&2
    print_error "  The inventory is incomplete; the run exits 1. Common cause: TMPDIR too small for syft's layer cache."
fi
...
SBOM_FAILED_IMAGES="$(printf '%s\n' "${FAILED_IMAGES[@]}")" ENUM_DECLARED_PINS="${ENUM_DECLARED_PINS:-0}" ENUM_TRACKED="${ENUM_TRACKED:-0}" python3 - "$MANIFEST" "$TAG" "$GENERATED_AT" "$SYFT_VERSION" "$GRYPE_VERSION" "$GRYPE_DB_STATUS" "${IMAGES[@]}" <<'PY'
import json, os, sys
...
doc = {
    ...
    "grype_db": grype_db,
    "images": images,
    "enumeration": {"manifest_pins_declared": int(os.environ.get("ENUM_DECLARED_PINS", "0") or 0), "images_enumerated": int(os.environ.get("ENUM_TRACKED", "0") or 0), "note": "#2331: every declared manifest pin is enumerated regardless of the box's COMPOSE_PROFILES; compose rendered with --profile '*'; 0 declared = --images-from run"},
    "failed_images": [x for x in os.environ.get("SBOM_FAILED_IMAGES", "").split("\n") if x],
}
...
with open(out_path, "w") as fh:
    for r in sorted(rows):
        fh.write("\t".join(r) + "\n")
PY
        # #2322: a rescan that produced NO rows is not a previous side. It happens when the
        # previous images are not on this host and no committed baseline exists to gap-fill
        # (2026.08-ga.15 shipped without cve-baseline.tsv): the diff then labelled the whole
        # current inventory NEW (5632 findings, PERSISTENT 0) and exited 2 as if it were a
        # security block. An empty other side is "cannot diff", handled below.
        if [ -s "$PREV_RESCAN_TSV" ]; then
            PREV_SRC="$PREV_RESCAN_TSV"
        else
            print_warning "Rescan of the previous image set produced no rows (images not on this host, nothing committed to gap-fill) — no previous side."
        fi
    ...
    if [ -z "$PREV_SRC" ]; then
        # #2322: exit 1 (cannot diff), never 0 (looks clean) and never 2 (looks like a
        # security block). Phase 6b's gate needs a baseline; commit the previous release's
        # cve-baseline.tsv (this run writes one for THIS tag under $OUT_DIR).
        print_error "Previous-release CVE baseline not found under $PREV_BASE — the CVE diff cannot be computed (exit 1)."
        EXIT_RC=1
...
if [ "$EXIT_RC" -eq 0 ] && [ "${SCAN_FAILED:-0}" -eq 1 ]; then EXIT_RC=1; fi
_sbom_cleanup_tmpdir
exit "$EXIT_RC"
