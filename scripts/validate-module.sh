...
#    9. core/init-db.sh creates every database the module declares in
#       core/config/profiles.yaml (only if needs_postgres)
#   10. cli/init.sh preset wiring (best-effort: presence in any preset list;
#       the root razzfazz-init.sh shim moved to legacy/ in #119 — #1173)
#   11. cli/upgrade.sh data-migration block (best-effort grep)
#   12. cli/post-install.sh post-deploy block (verify_<module>() or a mention)
#   13. core/help/mirror_config.json has the entry
#  13b. docs/{enterprise,community}/**/<module>*.md exists (own docs are auto-
#       discovered from docs/, never core/help/own_docs — mirror_config.json policy)
...
#       [ENV: fleet] — gated on passwordless SSH to the fleet, and time-boxed.
#       Off-fleet it SKIPs with the reason instead of blocking (#1129).
#       Override: RAZZFAZZ_VALIDATE_PREPARE_RELEASE=1|0,
#                 RAZZFAZZ_VALIDATE_PREPARE_RELEASE_TIMEOUT=<seconds>.
...
# -----------------------------------------------------------------------------
# The databases a module owns are declared in profiles.yaml (the same authority
# that decided NEEDS_PG above) — check for THOSE, not for the module id. #1129:
# this probe used to grep core/init-db.sh case-sensitively for the lowercase
# module id, but the repo names databases with UPPERCASE shell variables
# ($GITEA_DB, ${AGENT_MANAGER_DB:-agent_manager_db}). Measured on main, that
# false-FAILed gitea/chat/agents — all three of which do create their database —
# while onyx/cognee/lightrag passed only because their names happen to appear in
# a comment or a helper-function name. A mention is not an entry, so what is
# required here is a `generate_*db_sql` CALL for each declared database.
if [ "$NEEDS_PG" = "true" ]; then
    if [ ! -f core/init-db.sh ]; then
        emit FAIL "9.init-db" "needs_postgres=true but core/init-db.sh is missing"
    else
        db_probe=$(MODULE="$MODULE_ID" python3 - <<'PY' 2>/dev/null
import os, re, sys, yaml

m = os.environ['MODULE']
try:
    p = yaml.safe_load(open('core/config/profiles.yaml', encoding='utf-8')) or {}
    e = (p.get('profiles') or {}).get(m) or {}
    dbs = [d for d in (e.get('databases') or []) if isinstance(d, str)]
    if not dbs:
        print("ERROR no databases declared in profiles.yaml"); sys.exit(0)
    src = open('core/init-db.sh', encoding='utf-8').read()

    # db name -> the variable(s) that can carry it. Convention is the
    # uppercased name ($GITEA_DB for gitea_db); the .env.example allocation
    # table catches the ones that are named differently (dify_plugin_db is
    # $DB_PLUGIN_DATABASE), so no alias list has to be maintained here.
    try:
        env_src = open('config/.env.example', encoding='utf-8').read()
    except OSError:
        env_src = ''

    missing = []
    for db in dbs:
        names = {db.upper()} | {mm.group(1) for mm in re.finditer(
            r'^\s*([A-Z0-9_]+)=%s\s*$' % re.escape(db), env_src, re.M)}
        pat = re.compile(r'generate_[a-z_]*db_sql\s+"\$\{?(?:%s)\b'
                         % '|'.join(re.escape(n) for n in sorted(names)))
        if not pat.search(src):
            missing.append(db)
    if missing:
        print("MISSING no CREATE DATABASE call in core/init-db.sh for: "
              + ", ".join(missing))
    else:
        print("OK creates " + ", ".join(dbs))
except Exception as exc:      # noqa: BLE001 — probe must never abort the run
    print("ERROR %s" % exc)
PY
)
        case "$db_probe" in
            OK\ *)      emit PASS "9.init-db" "${db_probe#OK }" ;;
            MISSING\ *) emit FAIL "9.init-db" "${db_probe#MISSING }" ;;
            *)          emit WARN "9.init-db" "could not verify core/init-db.sh (${db_probe#ERROR })" ;;
        esac
    fi
else
...
# -----------------------------------------------------------------------------
# Touchpoint 10: cli/init.sh preset wiring (best-effort)
# #1173: the root razzfazz-init.sh is a legacy/ shim since #119 — the presets
# live in cli/init.sh (_pick_llm_profile_token, the four --package presets).
# -----------------------------------------------------------------------------
if [ -f cli/init.sh ]; then
    if grep -qE "['\",](${MODULE_ID})['\",]|profiles.*${MODULE_ID}|\"${MODULE_ID}\"" cli/init.sh; then
        emit PASS "10.init.sh-preset" "found in cli/init.sh preset(s)"
    else
        emit INFO "10.init.sh-preset" "not in any cli/init.sh preset (intentional if operator-only-enable)"
    fi
else
    emit WARN "10.init.sh-preset" "cli/init.sh missing"
fi

# -----------------------------------------------------------------------------
# Touchpoint 11: cli/upgrade.sh data-migration (best-effort) — #1173: cli/, not the shim
# -----------------------------------------------------------------------------
if [ -f cli/upgrade.sh ] && grep -qE "${MODULE_ID}|${ID}" cli/upgrade.sh; then
    emit PASS "11.upgrade.sh-migration" "module mentioned in cli/upgrade.sh"
else
    emit INFO "11.upgrade.sh-migration" "no migration block (acceptable if no first-run state needed)"
fi

# -----------------------------------------------------------------------------
# Touchpoint 12: cli/post-install.sh — #1173: the verify_<module>() idiom is the
# real post-deploy block; a bare mention is the weaker, still-accepted form.
# -----------------------------------------------------------------------------
if [ -f cli/post-install.sh ]; then
    _mod_fn="$(printf '%s' "${MODULE_ID}" | tr '-' '_')"
    if grep -qE "^(verify|configure|post_install)_${_mod_fn}\(\)" cli/post-install.sh; then
        emit PASS "12.post-install" "verify/configure block: $(grep -oE "^(verify|configure|post_install)_${_mod_fn}\(\)" cli/post-install.sh | head -1)"
    elif grep -qE "${MODULE_ID}|${ID}" cli/post-install.sh; then
        emit PASS "12.post-install" "module mentioned in cli/post-install.sh"
    else
        emit INFO "12.post-install" "no post-install block (acceptable if not needed)"
    fi
else
    emit WARN "12.post-install" "cli/post-install.sh missing"
fi
...
# -----------------------------------------------------------------------------
# Touchpoint 13b: own docs — #1173: core/help/own_docs/ is RETIRED (2026.07 docs
# rework). mirror_config.json's _own_docs_policy: own docs are docs/enterprise/
# **.md (and docs/community/**.md), baked into the help image and auto-
# discovered at runtime — "never this file". So look where they actually live.
# -----------------------------------------------------------------------------
_own_doc="$(find docs/enterprise docs/community -type f -iname "${MODULE_ID}*.md" 2>/dev/null | head -1)"
if [ -n "$_own_doc" ]; then
    emit PASS "13b.own_docs" "$_own_doc"
else
    emit WARN "13b.own_docs" "no docs/{enterprise,community}/**/${MODULE_ID}*.md (operator-facing description missing)"
fi
...
# I4: prepare-release.sh --check                                  [ENV: fleet]
#
# #1129: this used to be an unconditional inline call, and off-fleet it is not
# a check — it is a stall. prepare-release --check SSHes every box in
# $RAZZFAZZ_FLEET (Check 12, hot-patch reconciliation) and runs the whole
# acceptance+unit+api suite (Check 13). Both streams are redirected here, so on
# a machine without passwordless SSH to the fleet the validator sits silent for
# minutes and reads as a hang rather than as a missing environment.
#
# The contract: decide the environment FAST, SKIP loudly with the reason, and
# never block. Precedence:
#   RAZZFAZZ_VALIDATE_PREPARE_RELEASE=1  force on   (still time-boxed)
#   RAZZFAZZ_VALIDATE_PREPARE_RELEASE=0  force off
#   unset/empty                          auto-detect: fleet iff the first box
#                                        of ${RAZZFAZZ_FLEET:-prod-vm} answers
#                                        a BatchMode ssh within a few seconds
# The run itself is capped by RAZZFAZZ_VALIDATE_PREPARE_RELEASE_TIMEOUT (900s);
# hitting the cap is an environment verdict (SKIP), not a module defect (FAIL).
i4_run=""
i4_reason=""
TIMEOUT_CMD=""
command -v timeout >/dev/null 2>&1 && TIMEOUT_CMD="timeout 6"
case "${RAZZFAZZ_VALIDATE_PREPARE_RELEASE:-}" in
    1) i4_run=yes ;;
    0) i4_run=no; i4_reason="disabled via RAZZFAZZ_VALIDATE_PREPARE_RELEASE=0" ;;
    *)
        i4_box="${RAZZFAZZ_FLEET:-prod-vm}"
        i4_box="${i4_box%% *}"
        if ! command -v ssh >/dev/null 2>&1; then
            i4_run=no
            i4_reason="[ENV: fleet] no ssh client — off-fleet"
        elif ${TIMEOUT_CMD:-} ssh -o BatchMode=yes -o ConnectTimeout=3 \
                 -o StrictHostKeyChecking=no "$i4_box" true >/dev/null 2>&1; then
            i4_run=yes
        else
            i4_run=no
            i4_reason="[ENV: fleet] off-fleet: no passwordless SSH to '$i4_box'. prepare-release --check reconciles hot-patches across the fleet and runs the full test suite; running it here blocks for minutes with no output (#1129). Re-run on a release box, or force with RAZZFAZZ_VALIDATE_PREPARE_RELEASE=1"
        fi
        ;;
esac
...
elif [ "$i4_run" != "yes" ]; then
    emit SKIP "I4.prepare-release" "$i4_reason"
else
    ver=$(tr -d '[:space:]' < VERSION)
    i4_budget="${RAZZFAZZ_VALIDATE_PREPARE_RELEASE_TIMEOUT:-900}"
    if command -v timeout >/dev/null 2>&1; then
        timeout "$i4_budget" scripts/prepare-release.sh "$ver" --check >/dev/null 2>&1
        i4_rc=$?
    else
        scripts/prepare-release.sh "$ver" --check >/dev/null 2>&1
        i4_rc=$?
    fi
    case "$i4_rc" in
        0)         emit PASS "I4.prepare-release" "all checks pass" ;;
        124|137)   emit SKIP "I4.prepare-release" "[ENV: fleet] timed out after ${i4_budget}s — prepare-release --check needs a release box (raise RAZZFAZZ_VALIDATE_PREPARE_RELEASE_TIMEOUT if that is where you are)" ;;
        *)         emit FAIL "I4.prepare-release" "scripts/prepare-release.sh $ver --check exited $i4_rc" ;;
    esac
fi
