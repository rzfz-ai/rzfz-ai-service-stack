...
the **Settings portal** (`https://settings.<domain>/`; the former `config.<domain>` address redirects there since 2026.09).
...
rzfz init --domain mycompany.ai --timezone Europe/Berlin \
    --password 'SecurePass123!' --profiles chat,dify,llm-manager,llm-registry,llm-worker-agent,monitor \
    --hardware cpu --skip-interactive
```

Presets: `single-box` (AMD GPU, self-signed TLS), `master-cpu` (Let's Encrypt, Google
SSO), `testvm-cpu` (self-signed + Gitea), `worker-box` (GPUStack worker node only).
All three stack presets build a **LLM Manager** box; `worker-box` is the exception and
joins a remote GPUStack master. Run `rzfz init --help` for every flag.
...
| `llm-manager` + `llm-registry` + `llm-worker-agent` | LLM Manager, LiteLLM router, Zot registry, worker agent | The LLM front end of every box — one canonical OpenAI endpoint (`http://llm:8080/v1`) for chat, embeddings and rerank |
| `llm-legacy` | GPUStack v0.7.1 | OPTIONAL GPU backend behind the manager (`HARDWARE=amd\|nvidia`) |
...
> The manager trio is the front end and is always on; `llm-legacy` sits BEHIND it as an
> optional backend rather than beside it. `rzfz init` auto-detects `HARDWARE` and writes
> the matching `COMPOSE_FILE` overlay (`modules/llm/compose.devices.<amd|nvidia|cpu>.yml`).
>
> The `llm` and `llm-cpu` profiles were REMOVED in 2026.09 and `--llm-experimental`
> selects nothing; `rzfz upgrade` migrates an existing box automatically.
...
**Found a vulnerability?** Report it privately — see [SECURITY.md](SECURITY.md) for the
contact address, the supported versions and our disclosure policy. Please do not open a
