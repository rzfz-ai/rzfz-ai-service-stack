...
def _require_internal_token():
    ...
    shared-token gate is the PRIMARY control. As defense-in-depth, mcp-manager is
    deliberately NOT on `mcp-network` (#61 NEW-2, see compose.yml + docker_client)
    so a per-user proxy cannot even route to mcp-manager:5000 by container name.
