...
Mirrors agent-manager's Provisioner. For a given (user, mcp_id) it:
  ...
  4. registers an SSO-gated Caddy opaque-token route to the proxy
  5. records the instance; boot-reconcile re-registers routes after Caddy/box
     restarts (same self-heal as agent-manager).
...
class MCPProvisioner:
    ...
    def launch(self, mcp_id: str, user_id: str, username: str,
        ...
        if relaunch_of is not None:
            ...
            try:
                self._caddy.remove_route(mcp_id, instance_id)
            except Exception:
                logger.warning("relaunch: could not remove stale route for %s",
                               instance_id)
            try:
        ...
        try:
            ...
            # is on. `reach_backend: cognee` maps to `coding-agents` (where the
            # cognee service already lives). NEVER `default` (docker_client
            # refuses it) so the /internal isolation invariant holds.
            ...
            self._caddy.register_route(
                mcp_id, container_name, int(integ["container_port"]), instance_id,
                route_secret=route_secret,
                rewrite_upstream_host=bool(integ.get("rewrite_upstream_host")))
    ...
    def stop(self, instance_id, username: str) -> tuple:
        ...
        self._caddy.remove_route(inst["mcp_id"], instance_id)
    ...
    def reconcile_routes(self) -> dict:
        """Re-register the Caddy route for every running instance (boot self-heal).

        Same rationale as agent-manager: Caddy boots from its static Caddyfile,
        which doesn't carry these dynamic admin-API routes, so any Caddy restart
        / box reboot drops them. Idempotent; skips routes already present.
        """
        ...
        for inst in instances:
            ...
            summary["checked"] += 1
            try:
                if self._caddy.route_exists(inst["mcp_id"], inst["id"]):
                    summary["skipped"] += 1
                    continue
                if self._docker.get_container_state(inst["container_name"]) != "running":
                    summary["skipped"] += 1
                    continue
                integ = self._catalog.get(inst["mcp_id"])
                if not integ:
                    summary["skipped"] += 1
                    continue
                # Re-register with the SAME bearer the proxy was launched with,
                # so the route keeps enforcing it after a Caddy/box restart.
                cfg = inst.get("config") or {}
                route_secret = cfg.get("_route_secret") if isinstance(cfg, dict) else None
                ok = self._caddy.register_route(
                    inst["mcp_id"], inst["container_name"],
                    int(integ["container_port"]), inst["id"],
                    route_secret=route_secret,
                    rewrite_upstream_host=bool(integ.get("rewrite_upstream_host")))
                summary["reconciled" if ok else "failed"] += 1
            except Exception:
                summary["failed"] += 1
                logger.exception("reconcile_routes: failed for %s", inst.get("id"))
        if summary["reconciled"] or summary["failed"]:
            logger.info("mcp reconcile_routes summary: %s", summary)
        return summary
