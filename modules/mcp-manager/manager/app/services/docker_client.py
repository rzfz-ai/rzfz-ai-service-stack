...
#61 NEW-2, UPDATED for #619/#639: the proxy joins `mcp-network` and NOTHING
else (unchanged). mcp-manager now IS on mcp-network — since #619 its wildcard
bearer-relay replaced the dynamic Caddy routes, so the manager itself bridges
external traffic to each proxy and must resolve it by name. The old topology
control ("manager not on the proxy network") is replaced by explicit gates,
each fail-closed:
  * /internal/*  — X-MCP-Internal-Token shared secret (blueprints/internal.py)
  * dashboard/api/oauth — Caddy source-IP anchor (#639, app/__init__.py): a
    compromised proxy dialing mcp-manager:5000 with forged X-Authentik
    headers is refused before any header is read
  * instance hosts — the wildcard hook's own Caddy anchor + constant-time
    per-instance bearer (blueprints/wildcard_proxy.py); unknown hosts 404
The user's agents still reach the proxy THROUGH Caddy (the gated subdomain),
...
class MCPDockerClient:
    ...
    def create_container(self, name: str, image: str, environment: dict,
        ...
        # enumerate other users' proxies + bearer tokens. Since #619/#1019 Caddy
        # is NOT on mcp-network either (OPS-3: its unauthenticated admin API must
        # not share a net with these proxies); external requests reach a proxy via
        # Caddy → mcp-manager's wildcard bearer-relay (which IS on mcp-network),
        # never by direct container dial from Caddy.
