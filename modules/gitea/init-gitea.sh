...
if [ "${ENABLE_GITEA_AUTHENTIK_OIDC:-false}" = "true" ] && [ -n "${GITEA_OIDC_CLIENT_ID:-}" ] && [ ! -f "$AUTHENTIK_OIDC_MARKER" ]; then
    ...
    if su-exec git gitea admin auth add-oauth \
        ...
        --skip-local-2fa 2>&1; then
        touch "$AUTHENTIK_OIDC_MARKER" "$AUTHENTIK_OIDC_GRPMAP_MARKER"
...
elif [ "${ENABLE_GITEA_AUTHENTIK_OIDC:-false}" = "true" ] && [ -n "${GITEA_OIDC_CLIENT_ID:-}" ] && [ -f "$AUTHENTIK_OIDC_MARKER" ] && [ ! -f "$AUTHENTIK_OIDC_GRPMAP_MARKER" ]; then
    # Retrofit the #33 group->admin mapping onto an already-configured source.
    OIDC_SOURCE_ID=$(su-exec git gitea admin auth list 2>/dev/null | awk '$2=="Authentik"{print $1; exit}')
    if [ -n "$OIDC_SOURCE_ID" ]; then
        echo "Retrofitting Authentik OIDC group->admin mapping (source id ${OIDC_SOURCE_ID}, admin group: ${GITEA_SSO_ADMIN_GROUP})..."
        if su-exec git gitea admin auth update-oauth \
            --id "$OIDC_SOURCE_ID" \
            --scopes "openid" --scopes "email" --scopes "profile" --scopes "groups" \
            --group-claim-name "groups" \
            --admin-group "${GITEA_SSO_ADMIN_GROUP}" 2>&1; then
            touch "$AUTHENTIK_OIDC_GRPMAP_MARKER"
            echo "Authentik OIDC group->admin mapping applied"
        else
            echo "ERROR: Failed to retrofit Authentik OIDC group->admin mapping"
        fi
    else
        echo "WARNING: ENABLE_GITEA_AUTHENTIK_OIDC=true but no 'Authentik' OAuth source found to update — skipping group mapping retrofit"
    fi
