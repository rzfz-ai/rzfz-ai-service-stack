...
def make_docker_client(docker_host: str | None):
    ...
    # #2447: docker-py's default read timeout is 60 s. A box that has just
    # written 100+ GiB of weights can take longer than that to answer a
    # container create; the launch then fails on a daemon that is merely slow.
    timeout = docker_api_timeout()
    if docker_host:
        return docker.DockerClient(base_url=docker_host, timeout=timeout)
    return docker.from_env(timeout=timeout)


def docker_api_timeout() -> int:
    """Seconds the node waits for one docker API answer (RAZZFAZZ_DOCKER_API_TIMEOUT,
    default 180; a non-number or anything under 30 falls back to the default)."""
    raw = (os.environ.get("RAZZFAZZ_DOCKER_API_TIMEOUT") or "").strip()
    return int(raw) if raw.isdigit() and int(raw) >= 30 else 180
