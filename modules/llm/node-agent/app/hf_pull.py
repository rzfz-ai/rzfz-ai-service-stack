...
def download_stream(url: str, dest_path: str, *, http_stream, progress_cb=None,
                    ...
                    cancel_cb=None) -> int:
    """Stream ``url`` → ``dest_path`` atomically (temp file + os.replace). Returns
    the byte count. ``http_stream(url)`` yields ``(chunk_bytes, total_or_None)``.
    ``progress_cb(got, total)`` is called as bytes arrive. On any error the
    partial temp file is removed — never a half-written weight in place.
    ...
    written and a mismatch raises DigestMismatch BEFORE the file reaches its
    destination — a truncated transfer that ends cleanly or a corrupted proxy
    cache dies here instead of surfacing later as an engine crash-loop (#344).
    Same contract as puller.pull_blob, whose exception this reuses.

    NODE-5: this is a CORRUPTION/TRUNCATION check, not a tamper check, and the
    docstring used to claim otherwise. The digest comes from
    ``list_repo_entries`` — the same host, over the same ``urlopen`` that
    honours the same ``HTTPS_PROXY`` — so in ``proxied`` mode one
    TLS-intercepting middlebox serves both the bytes and the hash they are
    compared against. Real tamper-resistance needs a digest pinned by the
    manager's catalog, out of band of the pull. Tracked in #1037.
    """
    ...
    try:
        with os.fdopen(fd, "wb") as fh:
            for chunk, total in http_stream(url):
                # #302: checked per chunk — an undeployed instance must stop
                # streaming NOW, not when a multi-part quant finishes. The
                # raise lands in the cleanup below, so the temp file goes too.
                if cancel_cb is not None and cancel_cb():
                    raise PullCancelled(f"pull cancelled after {got} bytes: {url}")
                if not checked and total:
                    check_space_for(dest_dir, int(total), free_fn=free_fn)
                    checked = True
                if chunk:
                    fh.write(chunk)
