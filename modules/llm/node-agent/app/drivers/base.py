...
def llamacpp_task_flags(task: str | None, params: dict | None = None) -> list[str]:
    ...
    task. A plain chat deploy adds nothing (back-compat).
      ...
      embed  → ``--embeddings`` (pooling: the model's own, see below)
      rerank → ``--reranking``  (+ ``--pooling rank``)

    #2435: embed injects NO pooling. An embedding GGUF declares its pooling
    (``<arch>.pooling_type``: Qwen3-Embedding is LAST, nomic is mean) and
    llama.cpp applies it when the flag is absent. Forcing ``mean`` put every
    2026.09 embedding in a different space than the GPUStack-built indexes
    (cos 0.66-0.76 on 0.208) and flattened retrieval scores. A deployment that
    must keep another pooling (an index built under it) carries ``pooling`` in
    its params; the manager's migration 0024 pins ``mean`` on the deployments
...
#: #2447: pauses between engine-launch attempts after a TRANSIENT docker failure.
#: Measured on 0.175 (2026-09-23): two launches seconds after staging had written
#: 116 GiB into the models volume timed out at the socket proxy's 60 s read
#: timeout; the same call took 0.01 s minutes later. Three attempts over ~40 s of
#: pauses (plus the timeouts themselves) cover that write-back stall.
ENGINE_LAUNCH_RETRY_DELAYS = (10, 30)

#: Status codes from the socket proxy that mean "the daemon did not answer in
#: time", not "the request was wrong". 4xx (missing image, name conflict,
#: bad device) are verdicts and are raised at once.
_TRANSIENT_DOCKER_STATUS = frozenset({502, 503, 504})
_TRANSIENT_EXC_NAMES = frozenset({"ReadTimeout", "ConnectTimeout", "Timeout", "ConnectionError",
                                  "ProtocolError", "ReadTimeoutError", "RemoteDisconnected"})


def is_transient_docker_error(exc: BaseException) -> bool:
    """A transport stall between the node and the daemon (timeout, dropped
    connection, proxy 502/503/504). Matched by class name across the MRO so the
    check needs neither `requests` nor `docker` imported (fakes in tests, and the
    SDK wraps urllib3/requests errors inconsistently between versions)."""
    ...
    return code in _TRANSIENT_DOCKER_STATUS


def start_engine_retrying(spec: LaunchSpec, docker, *, sleep=None, start=None,
                          delays=ENGINE_LAUNCH_RETRY_DELAYS):
    """#2447: `start_engine`, retried on a transient docker failure.

    `containers.run` is create + start, so a timed-out call may have left a
    container behind. Before each retry the name is looked up: a RUNNING
    container is the launch that succeeded while the answer was lost, and is
    adopted; any other state is removed so the retry does not fail on the name.
    A non-transient error, or the last attempt's, is raised unchanged."""
    import time as _time

    sleep = sleep or _time.sleep
    start = start or start_engine
    attempt = 0
    while True:
        try:
            return start(spec, docker)
        except Exception as exc:
            if attempt >= len(delays) or not is_transient_docker_error(exc):
                raise
            logger.warning("engine launch of %s hit a transient docker failure (%s); "
                           "retrying in %ss (attempt %d of %d) (#2447)",
                           spec.name, type(exc).__name__, delays[attempt], attempt + 2, len(delays) + 1)
            sleep(delays[attempt])
            attempt += 1
            try:
                existing = docker.containers.get(spec.name)
            except Exception:
                existing = None
            if existing is not None:
                try:
                    ...
                    pass
                if getattr(existing, "status", None) == "running":
                    logger.warning("engine %s is running after all — adopting it (#2447)", spec.name)
                    return existing
                try:
                    existing.remove(force=True)
                except Exception:
                    logger.warning("could not remove half-launched %s before the retry", spec.name,
                                   exc_info=True)
