...
#
# SHARED by Dockerfile.vulkan (profile llm-legacy: 0.7.1 + AMD Vulkan) AND
# Dockerfile.cuda (profile llm-legacy + HARDWARE=nvidia since #1448:
# 0.7.1 + custom CUDA llama.cpp, #946). The
# arg-filtering below is engine-agnostic; the --no-mmap/--no-warmup rationale is
# written from the Strix-Halo case but the behaviour (mmap on, warmup on) is the
# same choice on the discrete-CUDA path, proven on the RTX PRO 6000 soak.
