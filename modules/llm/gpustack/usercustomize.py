...
# ---------------------------------------------------------------------------
# rc6.7 #48: load the PR #5255 server-side leak fix backport. Independent of
# the patches below; if the import fails the box still works (we just lose
# the fix) — never let bus_fix import errors break gpustack startup.
# ---------------------------------------------------------------------------
try:
    import usercustomize_bus_fix  # noqa: F401  imported for side effects
except Exception as e:
    print(
        f"[usercustomize] usercustomize_bus_fix import failed: {e!r}; "
        f"PR #5255 leak fix is NOT active",
        file=sys.stderr,
    )
