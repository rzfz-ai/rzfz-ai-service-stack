...
# #2324: on a cold box Open WebUI answers /health long before its API path is warm,
# and a single 15 s POST per function was the whole budget — 11 of 15 init
# scenarios ended "Installation Complete WITH FAILURES" (exit 6) with four of five
# function POSTs timed out and the fifth, seconds later, created. One bounded retry
# with backoff cures every one of them; nothing else about the seeding changes.
RETRY_ATTEMPTS = int(os.environ.get("SEEDER_RETRY_ATTEMPTS", "5"))
RETRY_BACKOFF_S = float(os.environ.get("SEEDER_RETRY_BACKOFF_S", "3"))


def _request_with_retry(method: str, url: str, *, what: str, **kwargs):
    """`requests.<method>` with a bounded retry on a transport error, a timeout or a
    5xx: attempts 1..RETRY_ATTEMPTS, sleeping RETRY_BACKOFF_S * 2**(n-1) between them.
    A 4xx is returned at once (the caller reports it); the last failure is raised."""
    ...
    for attempt in range(1, RETRY_ATTEMPTS + 1):
        try:
            ...
            if r.status_code < 500:
                return r
            last_exc = None
            log(f"[seeder] {what}: HTTP {r.status_code} on attempt {attempt}/{RETRY_ATTEMPTS}")
        except requests.RequestException as e:
            last_exc = e
            log(f"[seeder] {what}: {type(e).__name__} on attempt {attempt}/{RETRY_ATTEMPTS}")
...
def wait_for_openwebui() -> None:
    ...
    for attempt in range(1, WAIT_ATTEMPTS + 1):
        try:
            ...
            if r.status_code < 500:
                # #2324: /health answers before the API path is warm; the seeder
                # talks to /api/v1/*, so readiness is the API answering 200.
                api = requests.get(f"{OPENWEBUI_URL}/api/config", timeout=5)
                if api.status_code == 200:
                    log(f"[seeder] OpenWebUI API ready after {attempt} attempt(s).")
                    return
                log(f"[seeder] OpenWebUI /health ok but /api/config {api.status_code} — not ready yet.")
        ...
        try:
            ...
            if r.status_code < 500:
                api = requests.get(f"{OPENWEBUI_URL}/api/config", timeout=5)
                if api.status_code == 200:
                    log(f"[seeder] OpenWebUI API ready (no /health) after {attempt} attempt(s).")
                    return
        except requests.RequestException:
            pass
...
def seed_one(token: str, kind: str, file_path: Path) -> str:
    ...
    list_r = _request_with_retry(
        "get", f"{OPENWEBUI_URL}/api/v1/{kind}/list", what=f"list {kind}",
    ...
    if existing is None:
        ...
        r = _request_with_retry(
            "post", f"{OPENWEBUI_URL}/api/v1/{kind}/create", what=f"create {kind}/{fid}",
    ...
    r = _request_with_retry("post", update_url, what=f"update {kind}/{fid}", headers=headers, json=payload, timeout=15)
    if r.status_code in (200, 201):
