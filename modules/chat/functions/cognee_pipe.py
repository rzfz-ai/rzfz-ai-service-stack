...
# #249: keys under which cognee wraps the actual answer text, by observed
# response generations. 1.0 returned a bare list of strings; newer builds wrap
# each element in an object ({"search_result": [...]} per dataset with
# access-control on, or {"result"/"text"/"answer"/"completion": ...}), and
# some endpoints key the whole response by search type. Order matters —
# first present key wins.
_ANSWER_KEYS = ("search_result", "result", "text", "answer",
                "completion", "content")


def _extract_answer(out: Any) -> str:
    """Normalise any cognee /search response shape to readable answer text.

    The pre-#249 code did ``str(o)`` per list element, so a dict-wrapped
    answer rendered as a Python-repr'd JSON blob in the chat. Recurse into
    the known wrapper keys; degrade to compact JSON (never Python repr)
    only when nothing recognisable is found.
    """
    if out is None:
        return ""
    ...
    if isinstance(out, list):
        parts = [_extract_answer(o) for o in out]
        return "\n\n".join(p for p in parts if p).strip()
    if isinstance(out, dict):
        for key in _ANSWER_KEYS:
            if key in out and out[key] not in (None, "", [], {}):
                return _extract_answer(out[key])
        # search-type-keyed dict ({"GRAPH_COMPLETION": [...]}) — the value
        # carries the answer regardless of which type the caller used.
        for key, val in out.items():
            if key.isupper() and val not in (None, "", [], {}):
                return _extract_answer(val)
        try:
...
class Pipe:
    ...
    def _search(self, query: str, dataset: str) -> str:
        ...
        if r.status_code == 404:
            return ""
        r.raise_for_status()
        # #249: response shapes vary by cognee version (bare string list vs
        # dict-wrapped answers) — one extractor normalises them all.
        return _extract_answer(r.json())
