...
# Step 1 — KB
# ----------------------------------------------------------------------------
# #2185: the flag lives on the HOST, outside dify_db; it survived a full
# flatten on 0.79 and made this step print "already seeded" over an empty
# database. The flag is evidence only when the dataset it names still exists —
# ask Dify for it; an absent dataset (or an old flag without an id) reseeds.
_kb_flag_valid() {
    local id resp
    [ -f "$KB_FLAG" ] || return 1
    id=$(head -n1 "$KB_FLAG" 2>/dev/null | tr -d '[:space:]')
    [ -n "$id" ] || return 1
    resp=$(dify_api GET "/console/api/datasets/${id}" 2>/dev/null) || return 1
    printf '%s' "$resp" | python3 -c "import sys, json
try:
    d = json.load(sys.stdin)
except Exception:
    sys.exit(1)
sys.exit(0 if isinstance(d, dict) and str(d.get('id') or '') == '${id}' else 1)" 2>/dev/null
}
if _kb_flag_valid; then
    echo "[dify-seed] KB already seeded (flag: $KB_FLAG, dataset $(head -n1 "$KB_FLAG") present); skipping."
else
    if [ -f "$KB_FLAG" ]; then
        echo "[dify-seed] KB flag $KB_FLAG names dataset '$(head -n1 "$KB_FLAG" 2>/dev/null)' which Dify does not have (a leftover from a previous install, #2185) — reseeding."
        rm -f "$KB_FLAG"
    fi
