...
# The OpenAI-compatible base GPUStack serves (ends in /v1-openai). codex's
# Responses requests are translated to `${UPSTREAM}/chat/completions`.
UPSTREAM = os.environ.get("CODEX_SHIM_UPSTREAM", "http://gpustack:9090/v1-openai")
# The BARE origin (scheme+host[:port], no path). Used to reconstruct the full
# upstream URL for pass-through requests (opencode/gsd hit
# `/v1-openai/chat/completions` directly — those pass through unchanged, so we
# must forward to the same path under the bare origin, NOT double the
# `/v1-openai` prefix). Derived from UPSTREAM if not set explicitly.
...
class Handler(http.server.BaseHTTPRequestHandler):
    ...
    def _upstream_url(self) -> str:
        """Map the incoming request path to the full GPUStack upstream URL,
        against the BARE origin so we never double a path prefix.

          * codex hits `/v1/<x>`        → `${ORIGIN}/v1-openai/<x>`
          * opencode/gsd hit `/v1-openai/<x>` → `${ORIGIN}/v1-openai/<x>`
        """
        p = self.path
        if p.startswith("/v1/"):
            p = "/v1-openai/" + p[len("/v1/"):]
