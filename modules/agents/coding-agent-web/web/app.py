...
def create_session():
    ...
    # Launch the agent command inside the session (Phase 3 + 4).
    if launch and AGENT_LAUNCH_CMD:
...
INSTALL_SUGGESTIONS = [
    ...
    # install -g` works in the read-only-root sandbox and the tool persists.
    {
        ...
        "note": "Standalone Pi coding-agent CLI (pi.dev). After install, run "
                ...
                "GPUStack; ~/.pi/agent/models.json seeded for you). CLI command: "
                "`pi`.",
    },
    {
        # gsd-pi: the "Getting-Shit-Done" fork of Pi — the MAINTAINED, re-scoped
        ...
        "name": "GSD Pi — Getting-Shit-Done fork of Pi",
        "cmd": "npm install -g @opengsd/gsd-pi",
        "note": "GSD Pi — the local-first Getting-Shit-Done fork of Pi. Run "
                "`gsd` to start; point it at the local model in `gsd`'s provider "
                "setup (GPUStack, http://gpustack:9090/v1-openai). CLI command: "
                "`gsd`.",
    },
    {
