...
BASE_URL="${OPENAI_BASE_URL:-http://gpustack:9090/v1-openai}"
...
case "${AGENT_KIND}" in
  codex|opencode|gsd-pi|user-defined)
    ...
    CODEX_SHIM_DISABLE_THINKING="${CODING_SHIM_DISABLE_THINKING:-1}" \
        /opt/webui-venv/bin/python3 /opt/coding-agent-web/local_model_shim.py \
        >> /tmp/local-model-shim.log 2>&1 &
    echo "[coding-agent] local-model shim started on 127.0.0.1:${SHIM_PORT}"
    ;;
esac
...
CODING_TOOLS_WEB_PORT="${CODING_TOOLS_WEB_PORT:-3004}" \
    /opt/webui-venv/bin/python3 /opt/coding-agent-web/app.py \
    >> /tmp/coding-agent-web.log 2>&1 &
