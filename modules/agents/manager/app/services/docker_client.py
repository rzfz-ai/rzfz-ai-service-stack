...
class AgentDockerClient:
    ...
    def create_container(self, name: str, image: str, version: str,
        ...
        if sandbox:
            ...
            # #215: reaping init as PID 1. The coding-agent entrypoint ends in
            # `exec tail -f /dev/null` — a NON-reaping PID 1. The tmux server,
            # the backgrounded local-model shim + Flask web UI, and per-session
            # PTY shells all leave orphaned/double-forked children that reparent
            # to that `tail` and become permanent zombies. With no reaper they
    ...
    def get_container_stats(self, container_id_or_name: str) -> dict | None:
