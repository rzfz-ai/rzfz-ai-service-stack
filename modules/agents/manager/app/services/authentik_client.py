...
class AuthentikClient:
    ...
    def _find_app(self, slug: str):
        """Return the application dict with EXACTLY this slug, or None.

        NB: the applications `?slug=` query is NOT a reliable exact filter in
        this Authentik version (it can return unrelated results with a 0 count),
        so we filter the results by exact slug ourselves rather than trusting
        results[0].
        """
        res = self._get("/api/v3/core/applications/", search=slug)
