...
def _coding_split_env(kind: str, with_model: bool) -> str:
    ...
    if with_model:
        env.update({
            ...
            'OPENAI_BASE_URL': 'http://gpustack:9090/v1-openai',
...
SEED_TYPES = [
    {
        ...
        # v2026.8.3 == hermes v0.20.0 (security roll-up: gateway credential-isolation,
        # DoS/ReDoS #76083, JWKS #75437, token-leak #60199). Prior: v2026.7.1 == v0.18.0. v0.18 moved to
        # an s6-overlay-supervised image (see modules/agents/hermes-agent/
        # Dockerfile) and made the dashboard auth gate MANDATORY on non-loopback
        # binds — satisfied by the HERMES_DASHBOARD_BASIC_AUTH_* env below.
        'version': 'v2026.8.3',
        ...
        'env_template': json.dumps({
            ...
            'GPUSTACK_API_KEY': '{{GPUSTACK_API_KEY}}',
            'GPUSTACK_BASE_URL': 'http://gpustack:9090/v1-openai',
        ...
        'command': ['sh', '-c',
                    ...
                    'hermes config set model.base_url http://gpustack:9090/v1-openai 2>/dev/null; '
                    'hermes config set model.api_key "${GPUSTACK_API_KEY}" 2>/dev/null; '
    ...
    # ── #36 coding-agent split — per-TYPE coding agents ──────────────────────
    {
        ...
        'description': 'opencode AI coding agent — multi-session tmux web terminal, '
                       'defaults to the local GPUStack model, per-session model/auth picker.',
    ...
    },
    {
        ...
        'env_template': json.dumps({
            ...
            # The Caddy sandbox-path matcher (via
            # caddy_client.register_route + the /sandbox/(\d+) regex
            # handler) routes that path back to
            ...
            'LLM_BASE_URL': 'http://gpustack:9090/v1-openai',
            'LLM_EMBEDDING_API_KEY': '{{GPUSTACK_API_KEY}}',
            'LLM_EMBEDDING_BASE_URL': 'http://gpustack:9090/v1-openai',
...
class AgentCatalog:
    ...
    def _seed(self):
        ...
        for type_data in SEED_TYPES:
            self._db.upsert_agent_type(type_data)
