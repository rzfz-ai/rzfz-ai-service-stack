...
def main():
    ...
    # #420: the ONLY supported CLI path to change MAIN_DOMAIN post-install.
    parser.add_argument('--set-domain', metavar='FQDN',
                        help="Change MAIN_DOMAIN and every derived *_DOMAIN var "
                             "(same derivation as the Config Portal). Requires a "
                             "stack restart; Authentik re-domains itself on the "
                             "next up (DOMAIN_CHANGED re-apply).")
    ...
    parser.add_argument('--set-gpustack-api-key', metavar='KEY', help="(llm-legacy) Set the GPUStack API key for model sync")
    parser.add_argument('--gpustack-api-key-status', action='store_true', help="(llm-legacy) Show GPUStack API key status")
    ...
    if args.set_domain:
        import re as _re
        fqdn = args.set_domain.strip().lower().rstrip('.')
        if not _re.fullmatch(r'(?=.{1,253}$)([a-z0-9]([a-z0-9-]{0,61}[a-z0-9])?\.)+[a-z]{2,63}|localhost', fqdn):
            print(f"ERROR: '{args.set_domain}' is not a valid FQDN.")
            sys.exit(2)
        updates = cm.update_domain(fqdn)
        if not updates:
            print(f"MAIN_DOMAIN is already '{fqdn}' — nothing to do.")
            sys.exit(0)
        print(f"\nDomain changed to {fqdn} — {len(updates)} variables updated:")
        for k, v in sorted(updates.items()):
            print(f"  {k}={v}")
        print("\nNext steps:")
        print("  1. docker compose up -d          (services pick up the new *_DOMAIN set)")
        print("  2. Authentik re-applies its blueprints automatically on the next up")
        print("     (init container detects the domain change).")
        print("  3. TLS: letsencrypt boxes need DNS for the new domain BEFORE step 1;")
        print("     internal-CA boxes re-issue automatically.")
        print("  4. Open WebUI (if Chat was already in use): OWUI persists webui.url +")
        print("     oauth.provider_url in its OWN database and overrides the env —")
        print("     run the re-domain from the Config Portal (Domain & TLS), which")
        print("     reconciles the OWUI DB, or SSO to Chat stays on the OLD domain")
        print("     (ga.15 #256/#257 lesson).")
        sys.exit(0)
    ...
    if args.status:
        ...
        # Helper to toggle. #465: mutual exclusion comes from profiles.yaml —
        # the SAME authority the Configuration Portal enforces (the old rule
        # here targeted a phantom profile and never fired, so two LLM
        # runtimes could be selected and collide on container_name, #278).
        from setup_lib.config_manager import _load_mutual_exclusions
        _excl = _load_mutual_exclusions()
        ...
        def toggle_profile(p, active_set):
            if p in active_set:
                ...
                active_set.add(p)
                for e in _excl.get(p, []):
                    if e in active_set:
                        active_set.discard(e)
                        print(f"  (removed conflicting profile: {e})")
        ...
        # #449: submit BOTH states for every offered profile — update_config
        # preserves anything not mentioned, so an explicit 'off' is the only
        # way a wizard-unchecked profile actually turns off.
        for p in available_profiles:
            update_data[f'profile_{p}'] = 'on' if p in active_profiles else 'off'
        ...
        if smtp_mode == 'relay':
            ...
            if not smtp_host or not smtp_user or not smtp_pass:
                ...
                print("    Or use the Configuration Portal at https://settings.<domain>/")
            else:
