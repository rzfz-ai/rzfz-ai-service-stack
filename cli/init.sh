...
if [ -f "$APPLIANCE_OFFLINE_PKG" ] && command -v docker >/dev/null 2>&1; then
    ...
    appl_avail_kb="$(df -Pk /var/tmp 2>/dev/null | awk 'NR==2{print $4}')" || true
    ...
    if [ -n "$appl_avail_kb" ] && [ -n "$appl_pkg_kb" ] && [ "$appl_avail_kb" -lt "$appl_pkg_kb" ]; then
        print_warning "Low free space in /var/tmp ($((appl_avail_kb/1024/1024)) GiB) for extracting baked images (~up to $((appl_pkg_kb/1024/1024)) GiB) — load may fail; init would then fall back to build/pull."
    fi
    appl_tmp="$(mktemp -d /var/tmp/rzfz-appliance-images.XXXXXX 2>/dev/null)" || appl_tmp=""
    if [ -n "$appl_tmp" ]; then
        # package = `tar czf - -C STAGING_DIR .` → members are ./images/*.tar.
        # Extract only the images subtree to keep the transient footprint down;
        # tolerate both ./images and images member prefixes (mirrors upgrade.sh).
        if tar xzf "$APPLIANCE_OFFLINE_PKG" -C "$appl_tmp" ./images 2>/dev/null \
           || tar xzf "$APPLIANCE_OFFLINE_PKG" -C "$appl_tmp" images 2>/dev/null; then
            appl_n=0; appl_fail=0
            for appl_img in "$appl_tmp"/images/*.tar; do
                [ -e "$appl_img" ] || { print_info "  Baked package carries no images/ subtree (images-only skipped) — leaving build/pull enabled."; break; }
            ...
            if [ "$appl_n" -gt 0 ]; then
                print_success "Loaded ${appl_n} baked image(s) (${appl_fail} failed) — skipping docker build/pull for this run."
            ...
            print_warning "Could not extract images/ from ${APPLIANCE_OFFLINE_PKG} — init will build/pull images as usual."
        fi
        rm -rf "$appl_tmp"
    else
        print_warning "mktemp failed in /var/tmp — skipping baked-image load; init will build/pull as usual."
    fi
