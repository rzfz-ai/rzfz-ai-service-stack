...
_dify_install_plugin_from_package() {
    ...
    # already installed? (same list the marketplace path consults)
    local installed_id
...
try:
    ...
    for p in d.get('plugins',[]):
        if p.get('plugin_id')=='${search_name}' or p.get('name')=='${search_name}'.split('/')[-1]:
            print(p.get('plugin_unique_identifier','')); break
except Exception: pass" 2>/dev/null)
    if [ -n "$installed_id" ]; then
        print_info "Plugin '${search_name}' already installed (${installed_id}); the package carries ${rel}."
        rm -rf "$tmp"; return 0
    fi
...
try:
    ...
    print(s + '\t' + msgs)
        ...
        case "$status" in
            success) print_success "Plugin '${search_name}' installed from the offline package (${ident})."; return 0 ;;
            failed)  print_warning "Plugin '${search_name}' install FAILED in the daemon: ${detail:-no message} (#2222)"
