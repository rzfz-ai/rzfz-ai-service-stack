...
llm_manager_reconcile_consumers_late() {
    ...
    while [ "$waited" -lt "$LLM_MANAGER_LATE_READY_TIMEOUT" ]; do
        ...
        if [ -z "$pending" ]; then
            ...
            # #2448: the deploy-time verdict (llm-manager FAIL, e.g. a launch that
            # timed out, #2447) was measured before this recovery. The run has
            # now seen every deployment ready, so the manager's row says so —
            # otherwise post-install exits 1 "this box cannot serve models" on a
            # box that serves them. Only a verdict already recorded is replaced
            # (a --refresh run gains no new row); the timeout and #1760 branches
            # below never reach this line and keep the FAIL. The table is read
            # only when it IS the associative one: in an undeclared or indexed
            # array, `llm-manager` is arithmetic and `set -u` aborts on `llm`.
            if declare -p STEP_STATUS 2>/dev/null | grep -q '^declare -A' \
               && [ -n "${STEP_STATUS[llm-manager]:-}" ]; then
...
stage_baked_appliance_models() {
    ...
    # #2440: idempotency is per FILE against the package's models/ subtree
    # (index consulted, no scan) — a populated volume is NOT "already staged":
    # a package that ADDS a model or projector must still land it. Checked
    # BEFORE the expensive extract so a complete re-run stays instant.
    local _idx0 _prefix0 pkg_ggufs missing_ggufs n_pkg n_missing
    _idx0="$(appliance_pkg_index "$APPLIANCE_OFFLINE_PKG")" || _idx0=""
    pkg_ggufs=""; missing_ggufs=""
    if [ -n "$_idx0" ] && _prefix0="$(appliance_pkg_prefix "$_idx0" models)"; then
        pkg_ggufs=$(grep -E "^${_prefix0}/.*\.gguf$" "$_idx0" 2>/dev/null | sed 's#.*/##' | sort -u)
    fi
    if [ -n "$pkg_ggufs" ]; then
        missing_ggufs=$(razzfazz_models_volume_missing "$pkg_ggufs")
        n_pkg=$(printf '%s\n' "$pkg_ggufs" | sed '/^$/d' | wc -l | tr -d ' ')
        n_missing=$(printf '%s\n' "$missing_ggufs" | sed '/^$/d' | wc -l | tr -d ' ')
        if [ "$n_missing" -eq 0 ]; then
            print_info "All ${n_pkg} baked model GGUF(s) of the package are already in the box's models volume — nothing to stage."
            return 0
        fi
        print_substep "${n_missing} of ${n_pkg} baked model GGUF(s) are not in the models volume yet: $(printf '%s' "$missing_ggufs" | tr '\n' ' ')"
    elif razzfazz_models_volume_populated; then
        # No index (unreadable package): the old coarse rule, said out loud.
        print_warning "Package index unavailable — cannot compare per file; the models volume is populated, so staging is skipped (re-check with 'rzfz verify-models')."
        return 0
    fi
    ...
    if [ -n "$_idx" ] && tar xzf "$APPLIANCE_OFFLINE_PKG" -C "$tmp" "$_prefix" 2>/dev/null; then
        ...
        # #2440: stage only what the volume lacks — a file already present (by
        # name) is not copied again, so a re-run touches nothing it need not.
        if [ -n "${missing_ggufs:-}" ]; then
            local _f _b
            while IFS= read -r _f; do
                _b=$(basename "$_f")
                printf '%s\n' "$missing_ggufs" | grep -qxF "$_b" || rm -f "$_f"
            done < <(find "$tmp/models" -type f -name '*.gguf' 2>/dev/null)
        fi
...
# #2460: the hardware the catalogue is evaluated for. The box's .env HARDWARE,
# EXCEPT on a CPU box whose GPUStack has a READY worker with GPU devices — a
# master that serves through GPU workers (prod: HARDWARE=cpu, qwen3.6 on the AMD
# worker 10.163) is the cluster's family, or the #2158 CPU catalogue scales its
# running chat model to 0 and swaps every consumer to the 4B. An exported
# RZFZ_CATALOG_HARDWARE (set once by sync_consumers) wins. Silent: callers use
# it inside $(...).
_catalog_hardware() {
    if [ -n "${RZFZ_CATALOG_HARDWARE:-}" ]; then
        printf '%s' "$RZFZ_CATALOG_HARDWARE"; return 0
    fi
    local hw workers
    hw=$(read_env_value "$ENV_FILE" HARDWARE 2>/dev/null) || hw=""
    workers=$(gpustack_api GET "$(gpustack_api_prefix)/workers" 2>/dev/null) || workers=""
    HW="$hw" W="$workers" YAML_PATH="$SCRIPT_DIR/core/llm/standard-models.yaml" python3 -c "
import json, os, sys
sys.path.insert(0, os.path.dirname(os.environ['YAML_PATH']))
import hardware_catalog as hc
try:
    items = (json.loads(os.environ.get('W') or '{}') or {}).get('items') or []
except Exception:
    items = []
print(hc.cluster_family(os.environ.get('HW', ''), items), end='')
" 2>/dev/null || printf '%s' "$hw"
}

# #2460: resolve the catalogue hardware ONCE in the current shell, export it for
# core/llm/sync.py and every later $(...) call, and say so when it differs from
# the box's HARDWARE — the operator sees why a CPU master keeps qwen3.6.
_export_catalog_hardware() {
    [ -n "${RZFZ_CATALOG_HARDWARE:-}" ] && return 0
    local _hw _chw
    _hw=$(read_env_value "$ENV_FILE" HARDWARE 2>/dev/null) || _hw=""
    _chw=$(_catalog_hardware)
    [ -n "$_chw" ] || return 0
    export RZFZ_CATALOG_HARDWARE="$_chw"
    if [ "$_chw" != "$_hw" ]; then
        print_info "HARDWARE=${_hw}, but GPUStack serves through a READY ${_chw} worker — the model catalogue is evaluated for ${_chw}, not the CPU master (#2460)."
    fi
}

# _model_replicas <name>: the replicas GPUStack reports for a registered model
# (empty when it is not registered). #2460: what the log says is MEASURED.
_model_replicas() {
    local n="$1" prefix
    prefix=$(gpustack_api_prefix)
    gpustack_api GET "${prefix}/models" | N="$n" python3 -c "
import sys, json, os
try:
    d = json.load(sys.stdin)
except Exception:
    d = {}
for m in d.get('items', []):
    if m.get('name') == os.environ['N']:
        print(m.get('replicas', 0)); break
" 2>/dev/null
}
...
_model_rows_for_preset() {
    ...
    hw=$(_catalog_hardware)   # #2460: the cluster's family, not the CPU master's
    ...
    for pass in true false; do
        while IFS= read -r _row; do
            ...
            if [ "$pass" = "false" ]; then
                ...
                # #2460: a model that EXISTED before this run is never scaled down.
                # The spare rule keeps a FRESH registration from loading next to
                # the chat model; it must not stop an operator's running
                # deployment (0.79, 2026-09-23: qwen3.6 1→0 while this line said
                # "leaving at 0"). The log reports the measured replicas.
                if [ "$_pre_exists" = true ]; then
                    local _rep
                    _rep=$(_model_replicas "$alias")
                    if [ "${_rep:-0}" = "0" ]; then
                        print_info "auto_start:false model '$alias' already present at 0 replicas (measured) — left as is."
                    else
                        print_info "auto_start:false model '$alias' already present with ${_rep} replica(s) (measured) — an existing deployment is never scaled down (#2460)."
                    fi
                else
...
_default_chat_alias() {
    ...
    a=$(HW="$(_catalog_hardware)" YAML_PATH="$SCRIPT_DIR/core/llm/standard-models.yaml" python3 -c "
import yaml, os, sys
sys.path.insert(0, os.path.dirname(os.environ['YAML_PATH']))
import hardware_catalog as hc
...
# #2420 — the embedding dimension is MEASURED from the model the consumer is
# configured with, and the pair is kept consistent on every post-install run,
# `--refresh` included.
#
# Measured on 0.208 after the 2026.09-ga.1 upgrade: COGNEE_EMBEDDING_MODEL was
# still the pre-2026-06-19 `nomic-embed-text` (768 dims, an explicit value the
# set-if-empty defaults never touch) while COGNEE_EMBEDDING_DIM read 4096; every
# vector write failed the pgvector check, cognee caught it "failed open", and
# cognify/search both returned 200 over a brain that was permanently empty.
# The upgrade's `--refresh` never reached update_env_model_config, so nothing
# ever compared the two. The same alias returns 2560 on prod's backend and 4096
# on 0.208's, so the number cannot be defaulted: it is asked of the endpoint
# that will serve the consumer, AFTER the models are up (the --preset path's
# probe runs before the engines exist and silently takes the manifest number),
# and when that endpoint does not serve the configured model at all, the model
# moves to the fleet default first.
# EMBED_DIM-BEGIN
# _measure_embedding_dim <alias> → prints the vector length the box's LLM
# endpoint emits for <alias>, or nothing. The manager (canonical, #1445) is
# asked first with the consumer key post-install minted; GPUStack second on a
# box that still runs it standalone.
_measure_embedding_dim() {
    local alias="$1" key dim=""
    [ -n "$alias" ] || return 0
    key="${LLM_MANAGER_OWUI_KEY:-}"
    [ -n "$key" ] || key=$(read_env_value "${ENV_FILE:-.env}" LLM_MANAGER_OWUI_KEY 2>/dev/null) || key=""
    if [ -n "$key" ]; then
        dim=$(curl -sf --max-time 30 -H "Authorization: Bearer $key" -H "Content-Type: application/json" \
            "http://127.0.0.1:${LLM_MANAGER_PORT:-8091}/v1/embeddings" \
            -d "{\"model\":\"${alias}\",\"input\":\"x\"}" 2>/dev/null \
            | python3 -c "import sys,json; d=json.load(sys.stdin); print(len(d['data'][0]['embedding']))" 2>/dev/null) || dim=""
    fi
    if ! printf '%s' "$dim" | grep -Eq '^[0-9]+$'; then
        dim=$(gpustack_api POST /v1-openai/embeddings "{\"model\": \"$alias\", \"input\": \"x\"}" 2>/dev/null \
            | python3 -c "import sys,json; d=json.load(sys.stdin); print(len(d['data'][0]['embedding']))" 2>/dev/null) || dim=""
    fi
    printf '%s' "$dim" | grep -Eq '^[0-9]+$' && printf '%s' "$dim"
    return 0
}

# reconcile_embedding_dims — for every RAG consumer whose profile is active,
# make <PREFIX>_EMBEDDING_DIM equal the measured length of <PREFIX>_EMBEDDING_MODEL;
# a configured model the endpoint does not serve is replaced by the fleet
# default alias first. Writes only on a difference; says what it did.
reconcile_embedding_dims() {
    local default_alias
    default_alias=$(_default_embedding_alias); [ -n "$default_alias" ] || default_alias="qwen3-embedding"
    local pair profile prefix model_key dim_key model dim measured changed=0
    for pair in "cognee:COGNEE" "lightrag:LIGHTRAG"; do
        profile="${pair%%:*}"; prefix="${pair#*:}"
        echo "${COMPOSE_PROFILES:-}" | grep -qw "$profile" || continue
        model_key="${prefix}_EMBEDDING_MODEL"; dim_key="${prefix}_EMBEDDING_DIM"
        model=$(read_env_value "$ENV_FILE" "$model_key"); dim=$(read_env_value "$ENV_FILE" "$dim_key")
        [ -n "$model" ] || model="$default_alias"
        measured=$(_measure_embedding_dim "$model")
        if [ -z "$measured" ] && [ "$model" != "$default_alias" ]; then
            # The configured model is not served here (0.208: nomic-embed-text
            # against a manager that serves qwen3-embedding). The default is.
            measured=$(_measure_embedding_dim "$default_alias")
            if [ -n "$measured" ]; then
                print_warning "  ${profile}: ${model_key}=${model} is not served by this box's LLM endpoint — switched to the fleet default ${default_alias} (#2420)."
                update_env_value "$ENV_FILE" "$model_key" "$default_alias"
                model="$default_alias"; changed=1
            fi
        fi
        if [ -z "$measured" ]; then
            print_warning "  ${profile}: could not measure the embedding dimension of ${model} — ${dim_key} left at '${dim}' (verify it matches the served model, #2420)."
            continue
        fi
        if [ "$dim" != "$measured" ]; then
            print_warning "  ${profile}: ${dim_key}=${dim:-<empty>} but ${model} emits ${measured}-dim vectors — realigned to ${measured} (#2420). Vectors indexed at the old width must be re-indexed."
            update_env_value "$ENV_FILE" "$dim_key" "$measured"
            [ "$prefix" = "LIGHTRAG" ] && update_env_value "$ENV_FILE" "LIGHTRAG_EMBEDDING_SEND_DIM" "false"
            changed=1
        else
            print_substep "  ${profile}: ${model} → ${measured} dims, ${dim_key} agrees."
        fi
    done
    [ "$changed" -eq 0 ] || print_success "Embedding model/dimension pairs realigned (consumers re-read .env at the restart below)."
    return 0
}
# EMBED_DIM-END
...
update_env_model_config() {
    ...
    if [ "${SKIP_MODELS:-false}" = false ]; then
        embed_dim=$(_measure_embedding_dim "$embed_alias") || true   # #2420: manager first, then GPUStack; #1076: bare assignment under set -e
    else
...
sync_consumers() {
    ...
    _export_catalog_hardware   # #2460: sync.py evaluates the catalogue for the cluster
...
# #2404 — the upstream Cognee development account must not be a standing
# credential on a delivered box.
#
# Measured on 0.208 (2026.09-ga.1): `default_user@example.com` / the upstream
# documented password authenticated with HTTP 200 and a usable session, the
# row was a SUPERUSER, and the shipped frontend pre-filled the pair on the
# sign-in form. cognee:8000 is reachable from every container on the stack
# network and on the loopback publication, neither of which passes Authentik.
# The stack's own identity is razzfazz-ai-admin@<domain>; the upstream row is
# a leftover of an earlier cycle's bootstrap and must not exist as a login.
#
# Contract: on every post-install run with cognee active, (1) the upstream
# account, if present, is DEACTIVATED and stripped of superuser (not deleted:
# cognee's data rows may reference it; an inactive fastapi-users row cannot
# log in and stays reversible); (2) every other superuser that is not the
# stack identity is REPORTED, never changed (an operator's own account is
# theirs); (3) the upstream pair is then probed and must be refused.
# COGNEE_DEFAULT_ACCOUNT-BEGIN
cognee_reconcile_default_account() {
    case "${COMPOSE_PROFILES:-}" in *cognee*) ;; *) return 0 ;; esac
    docker ps --format '{{.Names}}' 2>/dev/null | grep -qx postgres || return 0
    docker ps --format '{{.Names}}' 2>/dev/null | grep -qx cognee || return 0
    local db stack_id psql rows upstream="default_user@example.com"
    db=$(read_env_value "$ENV_FILE" COGNEE_DB 2>/dev/null || true); [ -n "$db" ] || db="cognee_db"
    stack_id="razzfazz-ai-admin@$(read_env_value "$ENV_FILE" MAIN_DOMAIN 2>/dev/null || true)"
    psql="docker exec postgres psql -U ${POSTGRES_USER:-docker} -d ${db} -tAc"
    rows=$($psql "SELECT email || '|' || is_superuser || '|' || is_active FROM users ORDER BY email;" 2>/dev/null | sed '/^$/d') || rows=""
    [ -n "$rows" ] || { print_substep "cognee: no users table readable — default-account check skipped."; return 0; }

    local line email su active others="" changed=0
    while IFS='|' read -r email su active; do
        [ -n "$email" ] || continue
        # `bool || text` is cast by Postgres to true/false, not the t/f a bare
        # boolean column displays as — comparing to "t" never matched a real row
        # (0.208, 2026-09-23: the reconcile ran and changed nothing). Accept both.
        case "$su" in t|true) su=t ;; *) su=f ;; esac
        case "$active" in t|true) active=t ;; *) active=f ;; esac
        if [ "$email" = "$upstream" ]; then
            if [ "$active" = "t" ] || [ "$su" = "t" ]; then
                print_warning "cognee: the upstream development account ${upstream} exists (superuser=${su}, active=${active}) — deactivating it and removing superuser (#2404)."
                $psql "UPDATE users SET is_active = false, is_superuser = false WHERE email = '${upstream}';" >/dev/null 2>&1 \
                    && changed=1 \
                    || print_warning "cognee: could not deactivate ${upstream} — check the users table by hand (#2404)."
            fi
        elif [ "$su" = "t" ] && [ "$email" != "$stack_id" ]; then
            others="${others}${others:+, }${email}"
        fi
    done <<< "$rows"
    [ -z "$others" ] || print_warning "cognee: superuser(s) besides the stack identity ${stack_id}: ${others} — not changed; remove or downgrade them by hand if they are not yours (#2404)."

    # Probe: the upstream pair must be refused (the stack admin's own login,
    # which post-install performs right after this, is the separating control).
    local res status
    res="$(_cognee_login "$upstream" "default_password")"; status="${res%%|*}"
    if [ "$status" = "200" ]; then
        print_error "cognee: ${upstream} STILL authenticates after the reconcile — the box hands out a superuser session without Authentik (#2404). Deactivate the row by hand: UPDATE users SET is_active=false, is_superuser=false WHERE email='${upstream}';"
        return 1
    fi
    if [ "$changed" -eq 1 ]; then
        print_success "cognee: ${upstream} deactivated; the upstream pair is refused (HTTP ${status})."
    else
        print_substep "cognee: no upstream development account; the upstream pair is refused (HTTP ${status})."
    fi
    return 0
}
# COGNEE_DEFAULT_ACCOUNT-END

provision_mcp_registry() {
    case "${COMPOSE_PROFILES:-}" in *cognee*) ;; *) return 0 ;; esac
    print_step "MCP registry: provisioning cognee-mcp credentials..."
    # #2404: before minting anything, the upstream development account must
    # not be a login on this box. A failure here is reported, never fatal to
    # the refresh (the mint below still runs; the posture check in `rzfz
    # status` keeps it visible).
    cognee_reconcile_default_account || print_warning "cognee default-account reconcile reported a problem (see above)"
    local key_missing=0
...
"
    ...
    # #2415: saved apps that still point at the retired provider move to the one
    # whose models were just registered (or are reported, when nothing can move).
    _dify_migrate_graph_provider_refs "langgenius/gpustack/gpustack" "$provider"
    _dify_drop_inactive_provider_rows "$provider"
    [ "${_DIFY_REFS_REWRITTEN:-0}" -eq 0 ] || _dify_drop_provider_cache
    return "$_dcm_verdict"
}

# #2415 — a backend switch must take the OLD provider's REFERENCES with it.
#
# `_dify_drop_inactive_provider_rows` (below) removes the retired provider's
# model rows, and `dify_configure_manager_models` registers the served set
# under the active one. Neither touches what a saved app still POINTS AT: an
# LLM node in `workflows.graph`, a chat app's `app_model_configs.model`, a
# knowledge base's `datasets.embedding_model_provider`. A workflow authored
# against `langgenius/gpustack/gpustack` keeps that reference, the provider it
# names is now empty, and the node renders "Incompatible" with `rzfz status`
# green. Measured on 0.208 after the 2026.09-ga.1 upgrade: 5 of 7 apps; on prod
# (2026.08-ga.15) 16 of 41 apps would land in that state at the upgrade.
#
# Contract: rewrite the provider reference (stale → active) in the three
# columns, model names untouched, ONLY when the active provider has at least
# one registered model; keep the original text of every rewritten row in
# `rzfz_provider_ref_backup` (reversible by hand); then report what still does
# not resolve (a model name referenced by a graph that the active provider
# does not carry) — the report is the point even when nothing is rewritten.
# DIFY_MIGRATE_REFS-BEGIN
_dify_migrate_graph_provider_refs() {
    local stale="$1" active="$2"
    [ -n "$stale" ] && [ -n "$active" ] && [ "$stale" != "$active" ] || return 0
    docker ps --format '{{.Names}}' 2>/dev/null | grep -qx postgres || return 0
    local _psql="docker exec postgres psql -U ${POSTGRES_USER:-docker} -d ${DIFY_DB:-dify_db} -tAc"
    local _num='s/^[^0-9]*\([0-9][0-9]*\).*/\1/'

    # How many saved apps still point at the retired provider — the number the
    # operator needs whether or not anything can be rewritten (#2415 ask 3).
    local apps wf cfg ds bind
    _DIFY_REFS_REWRITTEN=0
    apps=$($_psql "SELECT count(DISTINCT app_id) FROM workflows WHERE graph LIKE '%${stale}%';" 2>/dev/null | tr -d '[:space:]')
    wf=$($_psql "SELECT count(*) FROM workflows WHERE graph LIKE '%${stale}%';" 2>/dev/null | tr -d '[:space:]')
    cfg=$($_psql "SELECT count(*) FROM app_model_configs WHERE model LIKE '%${stale}%';" 2>/dev/null | tr -d '[:space:]')
    ds=$($_psql "SELECT count(*) FROM datasets WHERE embedding_model_provider = '${stale}';" 2>/dev/null | tr -d '[:space:]')
    # #2427: the knowledge base's collection binding carries the provider id too.
    bind=$($_psql "SELECT count(*) FROM dataset_collection_bindings WHERE provider_name = '${stale}';" 2>/dev/null | tr -d '[:space:]')
    case "${apps:-}" in ''|*[!0-9]*) apps=0 ;; esac; case "${wf:-}" in ''|*[!0-9]*) wf=0 ;; esac
    case "${cfg:-}" in ''|*[!0-9]*) cfg=0 ;; esac; case "${ds:-}" in ''|*[!0-9]*) ds=0 ;; esac
    case "${bind:-}" in ''|*[!0-9]*) bind=0 ;; esac
    if [ "$apps" -eq 0 ] && [ "$cfg" -eq 0 ] && [ "$ds" -eq 0 ] && [ "$bind" -eq 0 ]; then
        _dify_report_unresolvable_datasets
        return 0   # nothing references the retired provider — silent on every --refresh
    fi
    print_step "Dify: ${apps} app(s) (${wf} workflow version(s)), ${cfg} chat config(s), ${ds} knowledge base(s), ${bind} collection binding(s) still reference ${stale##*/} (#2415, #2427)"

    local active_models
    active_models=$($_psql "SELECT count(*) FROM provider_models WHERE provider_name = '${active}';" 2>/dev/null | tr -d '[:space:]')
    case "${active_models:-}" in ''|*[!0-9]*) active_models=0 ;; esac
    if [ "$active_models" -eq 0 ]; then
        print_warning "  ${active##*/} has no registered models yet — references are NOT rewritten (they would point at another empty provider). Re-run 'rzfz post-install --refresh' once the models are served."
        return 0
    fi

    # #2427: a knowledge base moves only when the active provider serves ITS model
    # under the same name — then the stored vectors stay valid (same model, same
    # width). Any other dataset would need a re-index (#243); it is refused and
    # named, never repointed at a model that did not build its vectors.
    local _served="SELECT model_name FROM provider_models WHERE provider_name = '${active}'"
    local refused
    refused=$($_psql "SELECT name || ' (' || coalesce(embedding_model,'?') || ')' FROM datasets WHERE embedding_model_provider = '${stale}' AND coalesce(embedding_model,'') NOT IN (${_served});" 2>/dev/null | sed '/^$/d' | tr '\n' ';' | sed 's/;$//; s/;/, /g')

    # Reversible: the original text of every row about to change, kept in Dify's
    # own database beside the data it belongs to.
    $_psql "CREATE TABLE IF NOT EXISTS rzfz_provider_ref_backup (id bigserial PRIMARY KEY, migrated_at timestamptz NOT NULL DEFAULT now(), source_table text NOT NULL, row_id text NOT NULL, column_name text NOT NULL, original text);" >/dev/null 2>&1 || {
        print_warning "  could not create rzfz_provider_ref_backup — references NOT rewritten (no backup, no rewrite)."; return 0; }
    $_psql "INSERT INTO rzfz_provider_ref_backup (source_table, row_id, column_name, original)
              SELECT 'workflows', id::text, 'graph', graph FROM workflows WHERE graph LIKE '%${stale}%'
              UNION ALL SELECT 'app_model_configs', id::text, 'model', model FROM app_model_configs WHERE model LIKE '%${stale}%'
              UNION ALL SELECT 'datasets', id::text, 'embedding_model_provider', embedding_model_provider FROM datasets WHERE embedding_model_provider = '${stale}' AND embedding_model IN (${_served})
              UNION ALL SELECT 'dataset_collection_bindings', id::text, 'provider_name', provider_name FROM dataset_collection_bindings WHERE provider_name = '${stale}' AND model_name IN (${_served});" >/dev/null 2>&1 || {
        print_warning "  backup of the referencing rows FAILED — references NOT rewritten."; return 0; }

    # The rewrite: the provider id inside the JSON (any spacing after the colon),
    # the plain column on datasets. Model names are untouched.
    local n_wf n_cfg n_ds n_bind
    n_wf=$($_psql "UPDATE workflows SET graph = regexp_replace(graph, '\"provider\":\\s*\"${stale}\"', '\"provider\": \"${active}\"', 'g') WHERE graph LIKE '%${stale}%';" 2>/dev/null | sed -n 's/^UPDATE //p')
    n_cfg=$($_psql "UPDATE app_model_configs SET model = regexp_replace(model, '\"provider\":\\s*\"${stale}\"', '\"provider\": \"${active}\"', 'g') WHERE model LIKE '%${stale}%';" 2>/dev/null | sed -n 's/^UPDATE //p')
    n_ds=$($_psql "UPDATE datasets SET embedding_model_provider = '${active}' WHERE embedding_model_provider = '${stale}' AND embedding_model IN (${_served});" 2>/dev/null | sed -n 's/^UPDATE //p')
    # A binding moves unless the active provider already holds one for the same
    # model and type: two bindings would be two collections, and choosing which
    # one a knowledge base keeps is not a guess this script makes.
    n_bind=$($_psql "UPDATE dataset_collection_bindings b SET provider_name = '${active}' WHERE b.provider_name = '${stale}' AND b.model_name IN (${_served})
                       AND NOT EXISTS (SELECT 1 FROM dataset_collection_bindings a WHERE a.provider_name = '${active}' AND a.model_name = b.model_name AND a.type = b.type);" 2>/dev/null | sed -n 's/^UPDATE //p')
    case "${n_wf:-}" in ''|*[!0-9]*) n_wf=0 ;; esac; case "${n_cfg:-}" in ''|*[!0-9]*) n_cfg=0 ;; esac; case "${n_ds:-}" in ''|*[!0-9]*) n_ds=0 ;; esac
    case "${n_bind:-}" in ''|*[!0-9]*) n_bind=0 ;; esac
    print_substep "  Rewrote ${n_wf} workflow version(s), ${n_cfg} chat config(s), ${n_ds} knowledge base(s), ${n_bind} collection binding(s) → ${active##*/} (originals in rzfz_provider_ref_backup)."
    _DIFY_REFS_REWRITTEN=$(( n_wf + n_cfg + n_ds + n_bind ))
    [ -z "$refused" ] || print_warning "  Knowledge base(s) NOT moved — ${active##*/} does not serve their embedding model, so their vectors need a re-index, not a repoint: ${refused}"
    if [ "$n_bind" -lt "$bind" ]; then
        print_warning "  $(( bind - n_bind )) collection binding(s) still name ${stale##*/}: ${active##*/} already has a binding for the same model, or does not serve it. Left for an operator; rows in dataset_collection_bindings."
    fi

    # What still cannot resolve: model names the graphs ask of the active
    # provider that it does not carry. Reported, never invented.
    local unresolved
    unresolved=$($_psql "SELECT DISTINCT m->>'name' FROM workflows w, jsonb_path_query(w.graph::jsonb, '\$.nodes[*].data.model') m
                          WHERE m->>'provider' = '${active}'
                            AND NOT EXISTS (SELECT 1 FROM provider_models pm WHERE pm.provider_name = '${active}' AND pm.model_name = m->>'name');" 2>/dev/null | sed '/^$/d' | tr '\n' ' ')
    if [ -n "$unresolved" ]; then
        print_warning "  Model name(s) referenced by saved workflows that ${active##*/} does not serve: ${unresolved}— those nodes stay unresolved until the model is deployed or the workflow is edited."
    else
        print_substep "  Every model referenced by a saved workflow is registered under ${active##*/}."
    fi
    _dify_report_unresolvable_datasets
    return 0
}

# #2427 ask 2: a knowledge base whose (provider, model) pair has no model row
# looks healthy — documents, chunk count — until someone retrieves, and then
# says "invalid credentials" about a provider nobody uses any more. Name them.
_dify_report_unresolvable_datasets() {
    local _psql="docker exec postgres psql -U ${POSTGRES_USER:-docker} -d ${DIFY_DB:-dify_db} -tAc" dead
    dead=$($_psql "SELECT d.name || ' (' || d.embedding_model_provider || ' / ' || d.embedding_model || ')' FROM datasets d
                   WHERE d.indexing_technique = 'high_quality'
                     AND NOT EXISTS (SELECT 1 FROM provider_models pm WHERE pm.provider_name = d.embedding_model_provider AND pm.model_name = d.embedding_model);" 2>/dev/null | sed '/^$/d' | tr '\n' ';' | sed 's/;$//; s/;/, /g')
    [ -z "$dead" ] || print_warning "  Dify knowledge base(s) whose embedding model no provider serves — retrieval fails with 'credentials is not initialized': ${dead} (#2427)"
    return 0
}

# #2427: Dify caches provider credentials and configurations in Valkey, and a
# database change does NOT take effect — not even across a restart — until the
# cached keys are gone. Called after the rewrite AND the retired rows' removal.
_dify_drop_provider_cache() {
    docker ps --format '{{.Names}}' 2>/dev/null | grep -qx valkey || {
        print_warning "  Valkey not running — Dify's provider cache was NOT cleared; the rewritten rows take effect once it is (run 'rzfz post-install --refresh' again)."; return 0; }
    local pw db pat keys n=0 failed=0
    pw=$(read_env_value ".env" VALKEY_PASSWORD 2>/dev/null || true)
    db=$(read_env_value ".env.dify" REDIS_DB 2>/dev/null || true)
    case "${db:-}" in ''|*[!0-9]*) db=0 ;; esac
    local _vk=(docker exec valkey valkey-cli --no-auth-warning -n "$db")
    [ -z "$pw" ] || _vk=(docker exec valkey valkey-cli -a "$pw" --no-auth-warning -n "$db")
    for pat in 'provider_*credential*' 'provider_configurations:*'; do
        keys=$("${_vk[@]}" --scan --pattern "$pat" 2>/dev/null) || { failed=1; continue; }
        [ -n "$keys" ] || continue
        # shellcheck disable=SC2086  # one key per word; Dify's keys carry no whitespace
        if "${_vk[@]}" UNLINK $keys >/dev/null 2>&1; then
            n=$(( n + $(printf '%s\n' "$keys" | sed '/^$/d' | wc -l) ))
        else
            failed=1
        fi
    ...
    if [ "$failed" -ne 0 ]; then
        print_warning "  Dify's provider cache could not be cleared completely — the rewritten rows may look unchanged until it is (re-run 'rzfz post-install --refresh')."
    else
        print_substep "  Cleared ${n} cached Dify provider key(s) so the rewritten rows take effect (#2427)."
    fi
    return 0
}
# DIFY_MIGRATE_REFS-END
...
refresh_ensure_default_models_deployed() {
    _export_catalog_hardware   # #2460: a CPU master serving through GPU workers is that GPU class
    ...
    default_aliases=$(HW="$(_catalog_hardware)" YAML_PATH="$SCRIPT_DIR/core/llm/standard-models.yaml" python3 -c "
import yaml, os, sys
sys.path.insert(0, os.path.dirname(os.environ['YAML_PATH']))
import hardware_catalog as hc
try:
...
" 2>/dev/null)
    ...
    if [ -z "$chat_alias" ]; then
        chat_alias=$(HW="$(_catalog_hardware)" YAML_PATH="$SCRIPT_DIR/core/llm/standard-models.yaml" python3 -c "
import yaml, os, sys
sys.path.insert(0, os.path.dirname(os.environ['YAML_PATH']))
import hardware_catalog as hc
spec = yaml.safe_load(open(os.environ['YAML_PATH'])) or {}
...
if [ "$DO_REFRESH" = true ]; then
    ...
    # #2420: the RAG consumers' embedding model/dimension pairs, measured against
    # the endpoint that serves them, now that the default models are up. The
    # upgrade's --refresh never reached update_env_model_config, so an aged box
    # kept a retired model beside a new dimension and an always-empty brain.
    print_step "Reconciling embedding model/dimension pairs (#2420)..."
    reconcile_embedding_dims || print_warning "embedding dim reconcile reported a problem (see above)"
