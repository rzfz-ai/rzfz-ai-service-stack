...
stage_baked_appliance_models() {
    ...
    if ! docker ps --format '{{.Names}}' 2>/dev/null | grep -qx gpustack; then
        print_warning "Baked appliance package present but gpustack isn't running — skipping model staging (re-run 'rzfz post-install' once the stack is up)."
        return 0
    fi
    # Idempotent: a populated $RAZZFAZZ_LOCAL_MODELS_DIR means we already staged.
    # (Checked BEFORE the expensive extract so re-runs are instant.)
    if docker exec gpustack sh -c "ls -A '$RAZZFAZZ_LOCAL_MODELS_DIR' 2>/dev/null | head -n1 | grep -q ." 2>/dev/null; then
        print_info "Baked model GGUFs already present in ${RAZZFAZZ_LOCAL_MODELS_DIR} — skipping staging."
        return 0
    fi
    ...
    if tar xzf "$APPLIANCE_OFFLINE_PKG" -C "$tmp" ./models 2>/dev/null \
       || tar xzf "$APPLIANCE_OFFLINE_PKG" -C "$tmp" models 2>/dev/null; then
        if ls "$tmp"/models/* >/dev/null 2>&1; then
            if docker exec gpustack mkdir -p "$RAZZFAZZ_LOCAL_MODELS_DIR" >/dev/null 2>&1 \
               && docker cp "$tmp/models/." "gpustack:${RAZZFAZZ_LOCAL_MODELS_DIR}/" >/dev/null 2>&1; then
                print_success "Baked model GGUFs staged into ${RAZZFAZZ_LOCAL_MODELS_DIR} — offline deploy will register them via source=local_path."
            else
                print_warning "Could not copy baked model GGUFs into the gpustack-data volume — offline model deploy may report missing GGUFs. Re-check with 'rzfz verify-models'."
            fi
        else
...
dify_ensure_admin() {
    ...
    if [ "$setup_status" != "finished" ]; then
        ...
        if [ "$setup_after" != "finished" ]; then
            ...
            docker exec dify-api curl -s -c /tmp/dify_setup.jar \
                -X POST http://localhost:5001/console/api/init \
                -H "Content-Type: application/json" \
                -d "{\"password\":\"${init_pass}\"}" >/dev/null 2>&1
            docker exec dify-api curl -s -b /tmp/dify_setup.jar \
                -X POST http://localhost:5001/console/api/setup \
                -H "Content-Type: application/json" \
                -d "{\"email\":\"${admin_email}\",\"name\":\"razzfazz.ai Admin\",\"password\":\"${admin_pass}\"}" >/dev/null 2>&1
            sleep 2
        fi
    fi
