...
_llmm_wait_ready() {
    ...
    local want_n waited=0 last=0 json report ready pending unplaceable
...
for name, want in sorted(rows.items()):
    ...
    human = "; ".join(diffs)
    if managed(d):
        body = json.dumps({"params": want["params"], "task": want["task"]}, separators=(",", ":"))
