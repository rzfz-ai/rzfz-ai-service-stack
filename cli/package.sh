...
# #2352: a linked worktree's `.git` is a FILE (`gitdir: …/.git/worktrees/<name>`),
# and so is a submodule checkout's, so a filesystem test for a DIRECTORY refuses
# exactly the separate-checkout shape #2338 asks for. Measured building 2026.09-ga
# on 0.91: the run died in 0 s with "Must be run from a git repository" while
# `git rev-parse --is-inside-work-tree` said `true` in the same directory.
# Ask git, not the filesystem: true for a clone, a linked worktree and a submodule
# checkout; false everywhere the old test was false; and unlike the old test it
# also rejects a `.git` DIRECTORY that is not a repository, which it accepted.
