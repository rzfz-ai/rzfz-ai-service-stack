...
# checkout) that has no .env. Mirrors the late `-h|--help` handler verbatim.
...
refuse_on_worker_node rotate-secret "The secrets it rotates live in the master's .env; a worker holds only its own enrolment key."
