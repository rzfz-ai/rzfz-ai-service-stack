...
migrate_compose_file_chain() {
    ...
    local env_file="${SCRIPT_DIR}/.env"
    [ -f "$env_file" ] || { print_substep "No .env — skipping."; return 0; }
    local hardware current desired
    ...
    desired="compose.yml"
    case "${hardware}" in
        amd|nvidia|cpu) desired="${desired}:modules/llm/compose.devices.${hardware}.yml" ;;
    ...
    if [ "$current" = "$desired" ]; then
        print_substep "  COMPOSE_FILE matches HARDWARE=$hardware (no change needed)."
        return 0
    fi
    print_substep "  COMPOSE_FILE drift detected:"
    print_substep "    current: $current"
    print_substep "    desired: $desired"
    if [ "$DRY_RUN" = "true" ]; then
        print_substep "  Would set COMPOSE_FILE=$desired"
        return 0
    fi
    update_env_value "$env_file" "COMPOSE_FILE" "$desired"
    print_substep "  Repaired."
}
migrate_compose_file_chain
