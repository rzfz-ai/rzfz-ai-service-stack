...
Corporate TLS-intercept-proxy + upgrade-hardening bugfix on ga.11. `requires_pull=true`; no image-version bumps (config/code fixes). A corporate-proxy box running Dify pulls one new box-local overlay image — `ubuntu/squid` (digest-pinned).

### Fixed
- **Corporate proxy:** comprehensive internal-bypass (NO_PROXY) across all profiles + host gateway (#280); Dify plugin-marketplace SSRF **chaining sidecar** preserves the internal deny-list while routing public egress via the corporate proxy (#277); agent-provisioner injects proxy env + trust CA into runtime-spawned per-user agent containers (#281); channel-aware config-portal version-manifest fetch (#275).
- **Healthchecks:** cognee / matrix / tika no longer report a false "unhealthy" behind a daemon proxy (env-clear localhost probes) (#273).
- **GPUStack:** schema-line guard refuses to start against an incompatible database (#278); standalone embedded worker advertises a loopback IP so it registers reliably behind a proxy (#276).
- **cognee-frontend:** same-origin API base — no baked domain (#282).
- **Upgrade honesty:** offline `--package` upgrades record the correct version and `rzfz status` reports accurately (#272); `rzfz setup` preserves `COMPOSE_PROFILES` (#274); the model registry (distroless Zot) no longer shows a spurious "unhealthy".
- **PostgreSQL:** `mem_limit` 2g → 6g so `shared_buffers` has headroom for backend `work_mem` (prevents cgroup-OOM restart under load) + `stop_grace_period` 60s for a clean shutdown on host reboot (#251).

### Security
- **F-06 resolved** — the Dify SSRF-chain sidecar image is digest-pinned (`ubuntu/squid@sha256:…`), retiring the floating-tag supply-chain nuance; operator-overridable via `RAZZFAZZ_DIFY_SSRF_CHAIN_IMAGE`.
- Mode-A stack security assessment: 0 new High/Critical vs ga.11 (image pins byte-for-byte unchanged).
...
- **LightRAG v1.5.4 → v1.5.5** — ~8 advisories incl. auth-independent Stored-XSS (GHSA-xpjq-3w4w-w5wr), DoS, IPv6-SSRF, CVE-2026-61808; no re-index. `change_default LIGHTRAG_VERSION`.
