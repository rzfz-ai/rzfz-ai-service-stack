...
**Cycle status:** GA — latest `v2026.08-ga.15` (2026-08-21); `v2026.08-ga.14` (2026-08-21); `v2026.08-ga.13` (2026-08-20); `v2026.08-ga.12` (2026-08-13); `v2026.08-ga.11` (2026-08-06); `v2026.08-ga.10` (2026-08-05); `v2026.08-ga.9` (2026-08-03); `v2026.08-ga.8` (2026-08-03); `v2026.08-ga.7` (2026-08-02);
...
Per-tag notes: `releases/2026.08-ga.15/`, `releases/2026.08-ga.14/`, `releases/2026.08-ga.13/`, `releases/2026.08-ga.12/`, `releases/2026.08-ga.11/`, `releases/2026.08-ga.9/`, `releases/2026.08-ga.8/`, `releases/2026.08-ga.7/`, `releases/2026.08-ga.6/`, `releases/2026.08-ga.5/`, `releases/2026.08-ga.4/`, `releases/2026.08-ga.3/`, `releases/2026.08-ga.1/`, `releases/2026.08-ga/`.

## Patch — v2026.08-ga.15 (2026-08-21)
Offline + domain-change hardening on ga.14. No new modules, no env changes; safe
patch upgrade from ga.14. Rebuilds two images (`authentik-init`, `razzfazz-config`).

- **One-click domain change.** *Settings → Domain* previously only rewrote the
  derived `*_DOMAIN` vars and restarted Caddy, so a `MAIN_DOMAIN` change left the
  Authentik providers and Open WebUI's persisted login URL on the old host (404 /
  `ERR_SSL_PROTOCOL_ERROR`). A domain change now runs a full apply — reconcile the
  Open WebUI DB, re-template the Authentik blueprints (providers + outpost), clear
  the Authentik cache, and recreate each running domain-baking service — verified
  end-to-end on 0.91 with no manual database surgery.
- **Air-gapped boxes reconcile Authentik (#253).** The init step's `envsubst` +
  Docker CLI are baked into the `authentik-init` image instead of `apk add` at
  runtime, so the always-run outpost-binding reconcile works offline (was silently
  skipped → SSO-gated apps 404 after an offline upgrade or domain change).
- **Komodo monitoring connects (#252)** via the shared passkey the installer
  already provisions (was falling back to key auth → "invalid public key").
- **Smoother upgrades:** `--skip-host-updates` no longer stalls on a sudo prompt,
  and upgrades self-heal a legacy `COMPOSE_FILE` that could break a module's
  `docker compose` controls.

## Patch — v2026.08-ga.14 (2026-08-21)
Correctness bugfix on ga.13 folding the 2026.08 customer-delivery firefight fixes.
No new modules, no env changes, no Dockerfile changes; safe patch upgrade from ga.13.

- **OpenWebUI chat answers again.** Its socket.io token stream (`/ws/*`) sat behind
  the Authentik outpost, which a WebSocket cannot follow, so the handshake was
  `302`'d and chat "pulsed forever" with no answer — fleet-wide on every box (Dify
  unaffected). `/ws/*` is now exempt, the same posture as `/api/*`; OpenWebUI still
  authenticates the socket itself, so this is not an auth bypass.
- **The admin-password tool no longer reports a success it didn't achieve** — it
  re-verifies each app's new password against the live hash and fails loud, closing
  a gpustack/cognee lockout where the tool said "done" but the admin couldn't log in.
- **Changing a box's domain no longer breaks the SSO login redirect** (the outpost's
  browser-facing host is re-pinned), and the getting-started PDF is two pages again.
- **Release hygiene:** the installer's default-secret denylist is stored as hashes
  (no plaintext defaults in public source), and a pre-publish deliverable-review gate
  now runs before any public export.
- A day-1 regression tier now guards these delivery failure modes directly.

Per-tag detail: `releases/2026.08-ga.14/`.

## Patch — v2026.08-ga.13 (2026-08-20)
Security + correctness bugfix on ga.12. Supersedes the `ga.12-usbfix` respin (it contains
everything in it, including the appliance `--offline-package` support). No new modules, no
breaking changes.

**Security.** Four authentication/CSRF hardenings. The Configuration Portal (#377) and
agent-manager's `/api`, admin, dashboard and terminal WebSocket (#390, #397) now verify
that administrative requests genuinely arrive through the stack's reverse proxy before
trusting the caller's identity — closing an internal-network identity-spoofing class that
could reach the portal's secrets, module-toggle and backup/restore functions. #384
strengthens CSRF-token validation to a constant-time comparison. Also #388 and #378.
Exploit specifics are deliberately withheld from public notes; boxes on earlier 2026.08
patches should upgrade promptly.

**The stack stopped destroying its own module images.** Komodo ships `auto_prune` enabled,
so every box with the `monitor` profile ran `docker image prune -a -f` nightly at 00:00 UTC.
That deletes any image no container references — i.e. the custom-built images of *disabled*
modules, which on an offline box cannot be rebuilt or re-pulled (#539).

**Enabling a module now starts all of it.** The Configuration Portal's toggle started a
hand-maintained container list rather than the compose profile; the list had drifted in six
profiles, so modules came up partially and still reported success (#538).

**Cognee works again** — the embedding dimension was 768 while the fleet embedding model
emits 4096, so every vector insert failed silently (#413). Migrated on upgrade; see the
per-tag notes if cognee already holds data.

**Upgrades keep your compose overlays** — the `COMPOSE_FILE` self-heal used to rebuild the
chain from scratch, dropping the no-build, corporate-proxy and offline overlays (#448).

Also: 5.85 MB PNG background → 174 KB WebP (#526), licence page release ordering and BSL
Change Date (#524), coding-agent web-UI supervision (#255), plus #237, #238, #415.
`releases/2026.08-ga.13/`.
...
gateway; a **Dify marketplace egress sidecar** (keeps the stack's internal network
isolation intact, routes public egress via the corporate proxy — allowlist
...
(not MSC3861). **LightRAG v1.5.4 → v1.5.5** closes ~8 upstream security advisories
(incl. GHSA-xpjq-3w4w-w5wr and CVE-2026-61808) — no embedding/schema
