...
> **Patch v2026.08-ga.12 (2026-08-13):** **Corporate-proxy & upgrade hardening.** Boxes
> behind a corporate TLS-intercept proxy now work end-to-end: the plugin marketplace in the
> workflow builder (**Dify**) reaches its catalog through the proxy via a small egress-chaining
> sidecar that still enforces the internal deny-list, per-user **AI agents** receive the proxy
> settings + trust CA, and container healthchecks no longer flap behind a daemon proxy.
> Offline/air-gap `--package` upgrades now report the right version, and `rzfz setup` keeps
> your enabled module set intact. The database also gets more memory headroom so it can't be
> out-of-memory-restarted under load. Drop-in — a normal `rzfz upgrade` applies it; no config
> change, no re-index.
