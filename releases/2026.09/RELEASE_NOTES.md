...
`2026.09-ga.1` (2026-09-22) is a code-only patch: `rzfz upgrade` from 2026.09-ga
moves no image pin and needs no `.env` step; a first install whose model set
stayed incomplete is repaired by `rzfz post-install --preset standard`, which
now resumes a download from the bytes already held.

`2026.09-ga.2` (2026-09-23) is the upgrade-path patch: `rzfz upgrade` from 2026.09-ga.1,
2026.09-ga or 2026.08-ga.15 moves saved Dify apps and knowledge bases to the LLM
Manager, creates the per-service database roles, carries cognee's data into its
volumes and verifies the identity provider's configuration. A box installed fresh
on 2026.09 keeps its embedding pooling until its knowledge bases are re-indexed;
`rzfz status` checks every index against the engine. Backups are about half their
earlier size by design.

`2026.09-ga.3` (2026-09-25) fixes fresh installations: on 2026.09-ga.2 a new box stopped at `rzfz init` because cognee could not write its new storage volumes. Upgrades were not affected. A fresh 2026.09-ga.2 install that failed needs its two empty cognee volumes removed before `rzfz init` is run again (see the 2026.09-ga.3 notes).
...
- **Fixed in 2026.09-ga.3:** a fresh installation of 2026.09-ga.2 stopped at cognee (#2477).
- **Fixed in 2026.09-ga.2:** saved Dify apps and knowledge bases that kept the retired GPUStack provider (#2415, #2427); query embeddings computed with a different pooling than the stored documents (#2435); per-service database roles the upgrade did not create (#2401); cognee data stored in the container instead of its volumes (#2433); cognee's development account (#2404, security); plaintext partial backups (#2452, security); cognee volumes the new image could not write (#2455); a HARDWARE=cpu master with GPU workers losing its running models to the CPU model set on `post-install --refresh` (#2460). Do not run a refresh on such a box before upgrading to 2026.09-ga.2.
- **Fixed in 2026.09-ga.1:** a first install that downloads its models over the internet could end with no chat and no embedding model after a single stalled download; the node now resumes and retries, a weight that is complete launches even after a failed pull, and post-install stops waiting on a dead failed set (#2367). Also fixed there: the fresh-install seeder race and seed image (#2324, #2327, #2332) and the image readiness check failing a correct online box (#2368).
