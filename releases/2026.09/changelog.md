...
**2026.09-ga.2 (cut 2026-09-23).** Patch on 2026.09-ga.1, no image pin moves. Fixes: #2415 (saved Dify apps follow the backend switch, backed up in rzfz_provider_ref_backup), #2427 (knowledge-base collection bindings and Dify's provider cache), #2435 (embedding pooling follows the model; manager migration 0024 pins mean on pre-existing deployments; per-index status check), #2401 (per-service database roles created and verified before any recreate), #2433 (cognee storage roots follow the mounts; the upgrade preserves the live tree), #2420 (embedding model and dimension realigned, on --refresh too), #2403 (identity-provider configuration verified after the upgrade), #2416 (brand title), #2404 (cognee development account disabled, security), #2412 (customer copy), #2408 (usage page title), #2447 (engine launch retries a transient docker stall; 180 s docker timeout), #2448 (post-install re-records the manager verdict after its late pass), #2452 (partial backups encrypted, security), #2455 (cognee volumes follow the image user), #2460 (a running model is never scaled down; the model set follows the GPU workers of a CPU master), #2410 (day-one Dify journey). Measured on the fleet: the 2026.08-ga.15 → 2026.09 upgrade of the demo box 0.208, whose findings this patch fixes.

**2026.09-ga.3 (cut 2026-09-25).** Patch on 2026.09-ga.2: #2477 (the cognee image creates its storage roots owned by the cognee user; fresh `rzfz init` works again, Jira RZFZAI-2007).
